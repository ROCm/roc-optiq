                ...
                if (trace_event.isObject())
                {
                    ...
                    if (type.isString())
                    {
                        ...
                        else if (type_string == "X")
                        {
                            ...
                            {
                                rocprofvis_trace_process_t& process = trace_data[pid];
                                if (process.m_threads.find(tid) == process.m_threads.end())
                                {
                                    rocprofvis_trace_thread_t thread;
                                    thread.m_name = tid;
                                    process.m_threads[tid] = thread;
                                ...
                                {
                                    rocprofvis_trace_thread_t& thread = process.m_threads[tid];
                                    rocprofvis_trace_event_t event;
                                    event.m_name = name;
                                    event.m_start_ts = std::stod(ts);
                                    event.m_duration = std::stod(duration);
                                    trace_object.m_min_ts = std::min(trace_object.m_min_ts, event.m_start_ts);
                                    trace_object.m_max_ts = std::max(trace_object.m_max_ts, event.m_start_ts + event.m_duration);
                        ...
                        else if (type_string == "C")
                        {
                            ...
                            {
                                rocprofvis_trace_process_t& process = trace_data[pid];
                                if (process.m_threads.find(name) == process.m_threads.end())
                                {
                                    rocprofvis_trace_thread_t thread;
                                    thread.m_name = name;
                                ...
                                {
                                    rocprofvis_trace_thread_t& thread = process.m_threads[name];
                                    ...
                                    {
                                        counter.m_start_ts = object_map["ts"].getDouble();
                                    }
                                    ...
                                    if (args.isObject())
                                    {
                                        ...
                                        if (args_map.size())
                                        {
                                            ...
                                            if (value.isString())
                                            {
                                                ...
                                                counter.m_value = std::stod(value_str);
                                                thread.m_counters.push_back(counter);
                                                trace_object.m_min_ts = std::min(trace_object.m_min_ts, counter.m_start_ts);
                                                trace_object.m_max_ts = std::max(trace_object.m_max_ts, counter.m_value);
                                            }
