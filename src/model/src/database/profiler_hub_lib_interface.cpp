...
profiler_hub_result_t profiler_hub_get_time_slice(
	...
	profiler_hub_result_t result = kProfilerHubStatusUnknownError;
	try {
		result = profiler_hub::interface::GetTimeSlice(future_handle, trace, track_id, slice_container, timestamp_start, timestamp_end);
	}
	catch (std::exception& e)
	{
		spdlog::error(error_fmt, e.what());
...
profiler_hub_result_t profiler_hub_get_pmc_time_slice(
	...
	profiler_hub_result_t result = kProfilerHubStatusUnknownError;
	try {
		result = profiler_hub::interface::GetPmcTimeSlice(future_handle, trace, track_id, slice_container, timestamp_start, timestamp_end, left_neighbor, right_neighbor);
	}
	catch (std::exception& e)
	{
		spdlog::error(error_fmt, e.what());
