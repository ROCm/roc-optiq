...
TEST_CASE( "Data-Model Initialisation", "[require]" ) {
    ...
    if (trace)
    {
        ...
        if (db)
        {
            auto bind_result = rocprofvis_dm_bind_trace_to_database(trace, db);
...
TEST_CASE( "Read Database Metadata", "[require]" ) {
    PrintHeader("Create trace");
    rocprofvis_dm_trace_t trace = rocprofvis_dm_create_trace();
    REQUIRE (nullptr != trace);
    if (trace)
    {
        ...
        if (db)
        {
            auto bind_result = rocprofvis_dm_bind_trace_to_database(trace, db);
            REQUIRE(kRocProfVisDmResultSuccess == bind_result);
            if (kRocProfVisDmResultSuccess == bind_result)
            {
                PrintHeader("Allocate future");
                rocprofvis_db_future_t object2wait = rocprofvis_db_future_alloc(db_progress);
                REQUIRE(object2wait);
                if (nullptr != object2wait) 
                {
                    ...
                    auto meta_result = rocprofvis_db_read_metadata_async(db, object2wait);
...
TEST_CASE( "Database Load Random Slice", "[require]" ) {
    PrintHeader("Create trace");
    rocprofvis_dm_trace_t trace = rocprofvis_dm_create_trace();
    REQUIRE (nullptr != trace);
    if (trace)
        ...
        if (db)
        {
            ...
            if (kRocProfVisDmResultSuccess == bind_result)
            {
                ...
                if (nullptr != object2wait) 
                {
                    ...
                    if (kRocProfVisDmResultSuccess == meta_result)
                    {
                        ...
                        for (int i = 0; i < num_tracks; i++)
                        {
                            ...
                            char const* category = rocprofvis_dm_get_property_as_charptr(track, kRPVDMTrackCategoryEnumCharPtr, 0);
                            char const* process = rocprofvis_dm_get_property_as_charptr(track, kRPVDMTrackMainProcessNameCharPtr, 0);
                            char const* subname = rocprofvis_dm_get_property_as_charptr(track, kRPVDMTrackSubProcessNameCharPtr, 0);
                            REQUIRE(category);
                            REQUIRE(process);
                            REQUIRE(subname);
                        ...
                        if (num_tracks > 0)
                        {
                            ...
                            auto read_slice_result = rocprofvis_db_read_trace_slice_async(db, start_time, end_time, num_tracks, tracks_selection, object2wait);
                            REQUIRE(kRocProfVisDmResultSuccess == read_slice_result);
                            if (kRocProfVisDmResultSuccess == read_slice_result)
                            {
                                auto slice_wait_result = rocprofvis_db_future_wait(object2wait, UINT64_MAX);
                                    ...
                                    for (int i=first_track; (i < num_tracks) && (i < first_track+ LIST_SIZE_LIMIT); i++)
                                    {
                                        ...
                                        if (nullptr != slice)
                                        {
                                            ...
                                            for (int j = first_record; (j < num_records) && (j < first_record+ LIST_SIZE_LIMIT); j++)
                                            {
                                                ...
                                                if (track_category == rocprofvis_dm_track_category_t::kRocProfVisDmRegionTrack || track_category == rocprofvis_dm_track_category_t::kRocProfVisDmKernelTrack)
                                                {
                                                    int64_t duration = rocprofvis_dm_get_property_as_int64(slice, kRPVDMEventDurationInt64Indexed, j);
                                                    if (duration < 0)
                                                    {
                                                        printf(ANSI_COLOR_RED "Record %d has invalid duration %ld\n" ANSI_COLOR_RESET, j, duration);
                                                    }
                                                    ...
                                                    REQUIRE(type_str);
                                                    REQUIRE(symbol_str);
                                                    printf(ANSI_COLOR_BLUE "Record id=%ld, timestamp=%ld, op=%ld, op_str=%s, type=%s, symbol=%s\n", id, timestamp, op, op_str, type_str, symbol_str);
                                                    ...
                                                    auto result4flowtrace_wait = rocprofvis_db_future_wait(object2wait4flowtrace, UINT64_MAX);
                                                    REQUIRE(kRocProfVisDmResultSuccess == result4flowtrace_wait);
                                            
                                                    if (kRocProfVisDmResultSuccess == result4flowtrace && kRocProfVisDmResultSuccess == result4flowtrace_wait)
                                                    {
                                                        ...
                                                        if (flowtrace != nullptr) {
                                                            ...
                                                            printf(ANSI_COLOR_MAGENTA "Event has %ld data flow link:\n", num_endpoints);
                                                            for (int k = 0; k < num_endpoints; k++) {
                                                                ...
                                                                printf(ANSI_COLOR_MAGENTA "\tEndpoint %d at track=%ld, event_id=%ld, timestamp=%ld\n", k, track_id, event_id.bitfield.event_id, event_timestamp);
                                                            }
                                                    ...
                                                    auto stacktrack_wait = rocprofvis_db_future_wait(object2wait4stacktrace, UINT64_MAX);
                                                    REQUIRE(kRocProfVisDmResultSuccess == stacktrack_wait);
                                                    if (kRocProfVisDmResultSuccess == result4stacktrace && kRocProfVisDmResultSuccess == stacktrack_wait)
                                                    {
                                                        ...
                                                        if (stacktrace != nullptr) {
                                                            ...
                                                            printf(ANSI_COLOR_MAGENTA "Event has %ld stack frames:\n", num_frames);
                                                            for (int k = 0; k < num_frames; k++) {
                                                                ...
                                                                REQUIRE(frame_args);
                                                                REQUIRE(frame_code);
                                                                printf(ANSI_COLOR_MAGENTA "\tFrame %d : depth=%ld, symbol=%s, args=%s, code=%s \n", k, frame_depth, frame_symbol, frame_args, frame_code);
                                                            }
                                                    ...
                                                    auto extdata_wait = rocprofvis_db_future_wait(object2wait4extdata, UINT64_MAX);
                                                    REQUIRE(kRocProfVisDmResultSuccess == extdata_wait);
                                                    if (kRocProfVisDmResultSuccess == result4extdata && kRocProfVisDmResultSuccess == extdata_wait)
                                                    {
                                                        ...
                                                        if (extdata != nullptr) {
                                                            ...
                                                            printf(ANSI_COLOR_MAGENTA "Event has %ld extended data properties:\n", num_records);
                                                ...
                                                else if (track_category == rocprofvis_dm_track_category_t::kRocProfVisDmPmcTrack)
                                                {
                                                    double value = rocprofvis_dm_get_property_as_double(slice, kRPVDMPmcValueDoubleIndexed, j);
                                                    printf(ANSI_COLOR_BLUE "Record timestamp=%ld, value=%g\n", timestamp, value);
                                                }
                                            }
                                        }
                                        else
                                        {
                                            printf(ANSI_COLOR_RED "No time slice at %ld loaded for track %d\n",start_time,tracks_selection[i]);
                                        }
                                    }
                                }
                            }
                ...
                auto query_result = rocprofvis_db_execute_query_async(
                    db, "select * from top;", "Kernel execution summary", object2wait);
                ...
                if(kRocProfVisDmResultSuccess == query_result)
                {
                    ...
                    if(kRocProfVisDmResultSuccess == query_wait)
                    {
                        uint64_t num_tables = rocprofvis_dm_get_property_as_uint64(
                            trace, kRPVDMNumberOfTablesUInt64, 0);
                        if(num_tables > 0)
                        {
                            rocprofvis_dm_table_t table =
                                rocprofvis_dm_get_property_as_handle(
                                    trace, kRPVDMTableHandleIndexed, 0);
                            REQUIRE(table);
                            if(nullptr != table)
                            {
                                ...
                                for(int i = 0; i < num_rows; i++)
                                {
                                    ...
                                    if(table_row != nullptr)
                                    {
                                        uint64_t num_cells =
                                            rocprofvis_dm_get_property_as_uint64(
                                                table_row,
                                                kRPVDMNumberOfTableRowCellsUInt64, 0);
