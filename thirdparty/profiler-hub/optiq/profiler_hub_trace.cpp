...
namespace profiler_hub::interface
{
	...
	profiler_hub_result_t profiler_hub_trace_t::open_trace(future_t * future, uint32_t histogram_bucket_count) {
		...
		// calling back client
		// these calls can be done while initializing, reading and compiling data for profiling purposes
		// if this is done at the end, like in this example, it may make sense to introduce another callback to report progress
