    ...
    for(auto& item : cleanup_work.requests)
    {
        ...
        if(req.request_obj_handle)
        {
            ...
            if(req.request_type == RequestType::kExecuteScript)
            {
                ...
                // Cleanup runs off the UI thread after the tab is already
                // gone, so ProcessExecuteScriptRequest never fires. Queue
                // the same event so the editor drops out of Running.
                EventManager::GetInstance()->AddEvent(
                    std::make_shared<ScriptExecuteCompleteEvent>(
                        false, std::string(), "Trace closed",
                        cleanup_result.trace_file_path));
            }
#endif
