                             ...
                             "Compute Trace Controller Tests")
{
    ...
    // Fetches the mandatory PC-sampling data for the first kernel and validates
    // that the code-object and ISA-line collections are readable. The bundled
    // fixture declares the current schema version but predates the normalized
    // PC-sampling tables, so its asynchronous query reports an unknown error.
    // Richer input files return success and may contain ISA lines.
    // Fixture Reads: m_controller, m_workloads[0].handle, m_workloads[0].kernels[0].id
    SECTION("Controller Fetch Mandatory PC Sampling Data")
    {
        REQUIRE(!m_workloads.empty());
        REQUIRE(!m_workloads[0].kernels.empty());

        rocprofvis_handle_t* kernel_handle = nullptr;
        rocprofvis_result_t  result        = rocprofvis_controller_get_object(
            m_workloads[0].handle, kRPVControllerWorkloadKernelIndexed, 0,
            &kernel_handle);
        REQUIRE(result == kRocProfVisResultSuccess);
        REQUIRE(kernel_handle != nullptr);

        rocprofvis_handle_t* pc_sampling_handle = nullptr;
        result                                  = rocprofvis_controller_get_object(
            kernel_handle, kRPVControllerKernelPcSampling, 0, &pc_sampling_handle);
        REQUIRE(result == kRocProfVisResultSuccess);
        ...
        result = rocprofvis_controller_set_uint64(
            args, kRPVControllerPcSamplingArgsKernelId, 0, m_workloads[0].kernels[0].id);
        REQUIRE(result == kRocProfVisResultSuccess);
        ...
        result                 = rocprofvis_controller_get_uint64(
            pc_sampling_handle, kRPVControllerPCSamplingNumIsaLines, 0, &num_isa_lines);
        REQUIRE(result == kRocProfVisResultSuccess);

        uint64_t num_code_objects = 0;
        result = rocprofvis_controller_get_uint64(
            pc_sampling_handle, kRPVControllerPCSamplingNumCodeObjects, 0,
            &num_code_objects);
        REQUIRE(result == kRocProfVisResultSuccess);
        spdlog::info("Fetched mandatory PC-sampling data: {0} code objects, {1} ISA lines",
                     num_code_objects, num_isa_lines);

        if(num_isa_lines > 0)
        {
            REQUIRE(future_result == kRocProfVisResultSuccess);
            REQUIRE(num_code_objects > 0);

            uint64_t isa_line_id = 0;
            result               = rocprofvis_controller_get_uint64(
                pc_sampling_handle, kRPVControllerPCSamplingIsaLineId, 0, &isa_line_id);
            REQUIRE(result == kRocProfVisResultSuccess);
            REQUIRE(isa_line_id > 0);

            uint32_t instruction_length = 0;
            result                      = rocprofvis_controller_get_string(
                pc_sampling_handle, kRPVControllerPCSamplingIsaLineInstruction, 0,
                nullptr, &instruction_length);
            REQUIRE(result == kRocProfVisResultSuccess);
            REQUIRE(instruction_length > 0);

            uint64_t isa_code_object_id = 0;
            result = rocprofvis_controller_get_uint64(
                pc_sampling_handle, kRPVControllerPCSamplingIsaLineCodeObjectId, 0,
                &isa_code_object_id);
            REQUIRE(result == kRocProfVisResultSuccess);

            bool code_object_found = false;
            for(uint64_t i = 0; i < num_code_objects; i++)
            {
                uint64_t code_object_id = 0;
                result = rocprofvis_controller_get_uint64(
                    pc_sampling_handle, kRPVControllerPCSamplingCodeObjectId, i,
                    &code_object_id);
                REQUIRE(result == kRocProfVisResultSuccess);
                code_object_found |= code_object_id == isa_code_object_id;
            }
            REQUIRE(code_object_found);
        }
        else
        {
            REQUIRE((future_result == kRocProfVisResultSuccess ||
                     future_result == kRocProfVisResultUnknownError));
        }
