...
public:
    ...
    uint32_t GetMajorVersion() { return m_db_version.size() > 0 ? m_db_version[0] : 0; }
    uint32_t GetMinorVersion() { return m_db_version.size() > 1 ? m_db_version[1] : 0; }
    uint32_t GetPatchVersion() { return m_db_version.size() > 2 ? m_db_version[2] : 0; }
...
public:
    ...
    MetadataVersionControl(ProfileDatabase* db)
    : m_db(db)
    , m_rebuild_all(false)
    {}
    ...
    bool                   MustRebuild(uint32_t file_node_id, uint8_t id)
    {
        return m_rebuild_all || m_roc_optiq_table_properties[id].rebuild[file_node_id];
    };
    const char* GetTableName(uint8_t id)
    {
        return m_roc_optiq_table_properties[id].name.c_str();
    }
...
public:
    ...
    enum roc_optiq_table_dependency_mask_t : uint16_t
    {
        ...
        kRocOptiqTableDependentOnKernelDispatchLevel =
            1 << kRocOptiqTableKernelDispatchLevel,
        ...
        kRocOptiqTableDependentOnAllLevelTables =
            ...
            kRocOptiqTableDependentOnMemoryCopyLevel | kRocOptiqTableDependentOnTrackInfo,
    ...
    const char* GetHistogramTableName() override
    {
        return GetTableName(kRocOptiqTableHistogram);
    };
    const char* GetTrackInfoTableName() override
    {
        return GetTableName(kRocOptiqTableTrackInfo);
    };
    bool MustRebuildHistogram(uint32_t file_node_id) override
    {
        return MustRebuild(file_node_id, kRocOptiqTableHistogram);
    }
    bool MustRebuildTrackInfo(uint32_t file_node_id) override
    {
        return MustRebuild(file_node_id, kRocOptiqTableTrackInfo);
    }
};
...
public:
    ...
    enum roc_optiq_table_dependency_mask_t : uint16_t
    {
        ...
        kRocOptiqTableDependentOnAllLevelTables =
            kRocOptiqTableDependentOnKernelDispatchLevel |
            kRocOptiqTableDependentOnRegionLevel | kRocOptiqTableDependentOnTrackInfo,
        kRocOptiqTableDependentOnHistogram = 1 << kRocOptiqTableHistogram,
