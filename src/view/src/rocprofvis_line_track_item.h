...
private:
    ...
    // Guard before every use: the ctor asserts it is non-null, but that assert is
    // compiled out in release, so it may be null there if the cast ever fails.
    CounterTrackOptions* m_counter_options;

    // Cached interior Y-axis tick values, regenerated by UpdateYAxisTicks() only
    // when one of the cached inputs below changes.
