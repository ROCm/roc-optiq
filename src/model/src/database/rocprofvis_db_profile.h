...
public:
    ...
    , m_table_processor{ TableProcessor(this), TableProcessor(this), TableProcessor(this),
                         TableProcessor(this) } {};
    // ProfileDatabase destructor, must be defined as virtual to free resources of derived
    // classes
    ...
    rocprofvis_dm_result_t ReadTraceSlice(rocprofvis_dm_timestamp_t            start,
    ...
    // @param object - future object providing asynchronous execution mechanism
    // @return status of operation
    rocprofvis_dm_result_t ExecuteQuery(rocprofvis_dm_charptr_t query,
    ...
    static rocprofvis_dm_result_t DetectMultiNode(rocprofvis_db_filename_t  filename,
                                                  std::vector<std::string>& db_files);

    bool         isServiceColumn(const char* name);
    StringTable& StringTableReference() { return m_string_table; };

    // method to execute table database query with appropriate .CSV writer callback based
    // on existence of GROUP BY clause
    // @param query - database query
    // @param file_path output path to write .CSV
    // @param future - future object providing asynchronous execution mechanism
    // @return status of operation
    rocprofvis_dm_result_t ExportTableCSV(rocprofvis_dm_charptr_t query,
                                          rocprofvis_dm_charptr_t file_path,
                                          Future*                 future) override;

    virtual rocprofvis_dm_result_t         RemapStringId(uint64_t                    id,
                                                         rocprofvis_db_string_type_t type,
                                                         uint32_t node, uint64_t& result) = 0;
...
private:
    ...
    // Used in all-selected-tracks time slice query
    ...
    static int CallbackAddAnyRecord(void* data, int argc, sqlite3_stmt* stmt,
                                    char** azColName);

    static rocprofvis_dm_event_operation_t GetTableQueryOperation(std::string query);

    void BuildSliceQueryMap(slice_query_map_t&            slice_query_map,
                            rocprofvis_dm_track_params_t* props);

    bool IsEmptyRange(uint32_t track, uint64_t start, uint64_t end);

    // Searches for strings containing the passed in list of filter strings and builds a
    // WHERE IN clause for the table query.
...
protected:
    ...
    // @return status of operation
    rocprofvis_dm_result_t BuildTrackQuery(rocprofvis_dm_index_t   index,
    ...
    // @param slices - reference map array for storing slice handlers for multi-track
    // request
    // @return status of operation
    rocprofvis_dm_result_t BuildSliceQuery(rocprofvis_dm_timestamp_t       start,
                                           rocprofvis_dm_timestamp_t       end,
                                           rocprofvis_db_num_of_tracks_t   num,
                                           rocprofvis_db_track_selection_t tracks,
                                           rocprofvis_dm_string_t&         query,
    ...
    rocprofvis_dm_result_t BuildCounterSliceLeftNeighbourQuery(
        ...
        rocprofvis_dm_index_t track_index, rocprofvis_dm_string_t& query);
    ...
    rocprofvis_dm_result_t BuildTableQuery(
        ...
        uint64_t offset, bool count_only, rocprofvis_dm_string_t& query) override;
    ...
    rocprofvis_dm_result_t ExecuteQueriesAsync(
        ...
        std::vector<Future*>& futures, rocprofvis_dm_handle_t handle,
        RpvSqliteExecuteQueryCallback callback);

    rocprofvis_dm_result_t BuildComputeQuery(
        rocprofvis_db_compute_use_case_enum_t use_case, rocprofvis_db_num_of_params_t num,
        rocprofvis_db_compute_params_t params, rocprofvis_dm_string_t& query) override
    {
        (void) use_case;
        (void) num;
        (void) params;
        ...
        ROCPROFVIS_ASSERT_ALWAYS_MSG_RETURN(
            "Systems database does not build compute query",
            kRocProfVisDmResultNotSupported);
    }

    rocprofvis_dm_result_t ExecuteComputeQuery(
        rocprofvis_db_compute_use_case_enum_t use_case, rocprofvis_dm_charptr_t query,
        Future* future) override
    {
        (void) use_case;
        (void) query;
        (void) future;
        ROCPROFVIS_ASSERT_ALWAYS_MSG_RETURN(
            "Systems database does not support compute query",
            kRocProfVisDmResultNotSupported);
    }
...
protected:
    ...
    // @param azColName - pointer to column names
    // @return SQLITE_OK if successful
    static int CallbackCacheTable(void* data, int argc, sqlite3_stmt* stmt,
                                  char** azColName);
    // sqlite3_exec callback to process track information query and add track object to
    // Trace container
    // @param data - pointer to callback caller argument
    // @param argc - number of columns in the query
    // @param argv - pointer to row values
    // @param azColName - pointer to column names
    // @return SQLITE_OK if successful
    static int CallBackAddTrack(void* data, int argc, sqlite3_stmt* stmt,
                                char** azColName);
    // sqlite3_exec callback to load saved track information and add track object to Trace
    // container
    // @param data - pointer to callback caller argument
    // @param argc - number of columns in the query
    // @param argv - pointer to row values
    // @param azColName - pointer to column names
    // @return SQLITE_OK if successful
    static int CallBackLoadTrack(void* data, int argc, sqlite3_stmt* stmt,
                                 char** azColName);
    ...
    // @param azColName - pointer to column names
    // @return SQLITE_OK if successful
    static int CallbackAddFlowTrace(void* data, int argc, sqlite3_stmt* stmt,
                                    char** azColName);
    ...
    // @param azColName - pointer to column names
    // @return SQLITE_OK if successful
    static int CallbackAddExtInfo(void* data, int argc, sqlite3_stmt* stmt,
                                  char** azColName);
    ...
    // @param azColName - pointer to column names
    // @return SQLITE_OK if successful
    static int CallbackAddEssentialInfo(void* data, int argc, sqlite3_stmt* stmt,
                                        char** azColName);
    ...
    // @param azColName - pointer to column names
    // @return SQLITE_OK if successful
    static int CallbackAddArgumentsInfo(void* data, int argc, sqlite3_stmt* stmt,
                                        char** azColName);

    // sqlite3_exec callback to calculate graph level for an event and store it into trace
    // object map array
    // @param data - pointer to callback caller argument
    // @param argc - number of columns in the query
    // @param argv - pointer to row values
    // @param azColName - pointer to column names
    // @return SQLITE_OK if successful
    static int CalculateEventLevels(void* data, int argc, sqlite3_stmt* stmt,
                                    char** azColName);
    // sqlite3_exec callback to collect minimum/maximum timestamps and minimu/maximum
    // value/level
    // @param data - pointer to callback caller argument
    // @param argc - number of columns in the query
    // @param argv - pointer to row values
    ...
    static int CallBackLoadHistogram(void* data, int argc, sqlite3_stmt* stmt,
                                     char** azColName);
    ...
    virtual int ProcessTrack(rocprofvis_dm_track_params_t& track_params,
                             rocprofvis_dm_charptr_t*      newqueries) = 0;
...
protected:
    ...
    std::unordered_map<uint32_t, std::vector<rocprofvis_db_event_level_t>>
        m_event_levels[kRocProfVisDmNumOperation];
    std::unordered_map<uint32_t, std::unordered_map<uint64_t, size_t>>
                   m_event_levels_id_to_index[kRocProfVisDmNumOperation];
