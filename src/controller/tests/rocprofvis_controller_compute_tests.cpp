                             ...
                             "Compute Trace Controller Tests")
{
    ...
    // Verifies that PC-sampling data is loaded in four independent stages:
    // ISA first, then source, stall, and comment data on demand.
    SECTION("Controller Fetch PC Sampling Stages")
    {
        REQUIRE(!m_workloads.empty());
        WorkloadInfo& workload = m_workloads.front();
        REQUIRE(!workload.kernels.empty());

        const uint64_t       kernel_id     = workload.kernels.front().id;
        rocprofvis_handle_t* kernel_handle = nullptr;
        rocprofvis_result_t  result        = rocprofvis_controller_get_object(
            workload.handle, kRPVControllerWorkloadKernelById, kernel_id, &kernel_handle);
        REQUIRE(result == kRocProfVisResultSuccess);
        ...
        result = rocprofvis_controller_set_uint64(
            args, kRPVControllerPcSamplingArgsKernelId, 0, kernel_id);
        REQUIRE(result == kRocProfVisResultSuccess);
        result = rocprofvis_controller_set_uint64(
            args, kRPVControllerPcSamplingArgsSourceFileUuid, 0, 0);
        REQUIRE(result == kRocProfVisResultSuccess);
        ...
        {
            uint64_t instruction_line_count = 0;
            uint64_t source_file_count      = 0;
            uint64_t sample_state_count     = 0;
            REQUIRE(rocprofvis_controller_get_uint64(
                        pc_sampling_handle, kRPVControllerPCSamplingNumInstructionLines,
                        0, &instruction_line_count) == kRocProfVisResultSuccess);
            REQUIRE(rocprofvis_controller_get_uint64(
                        pc_sampling_handle, kRPVControllerPCSamplingNumSourceFiles, 0,
                        &source_file_count) == kRocProfVisResultSuccess);
            REQUIRE(rocprofvis_controller_get_uint64(
                        pc_sampling_handle, kRPVControllerPCSamplingNumPcSampleStates, 0,
                        &sample_state_count) == kRocProfVisResultSuccess);
            REQUIRE(source_file_count == 0);
            REQUIRE(sample_state_count == 0);

            future = rocprofvis_controller_future_alloc();
            REQUIRE(future != nullptr);
            REQUIRE(rocprofvis_controller_pc_sampling_fetch_source_async(
                        m_controller, args, future, pc_sampling_handle) ==
                    kRocProfVisResultSuccess);
            REQUIRE(rocprofvis_controller_future_wait(future, FLT_MAX) ==
                    kRocProfVisResultSuccess);
            REQUIRE(rocprofvis_controller_get_uint64(future, kRPVControllerFutureResult,
                                                     0, &future_result) ==
                    kRocProfVisResultSuccess);
            rocprofvis_controller_future_free(future);
            REQUIRE(future_result == kRocProfVisResultSuccess);

            uint64_t current_instruction_line_count = 0;
            REQUIRE(rocprofvis_controller_get_uint64(
                        pc_sampling_handle, kRPVControllerPCSamplingNumInstructionLines,
                        0, &current_instruction_line_count) == kRocProfVisResultSuccess);
            REQUIRE(current_instruction_line_count == instruction_line_count);
            REQUIRE(rocprofvis_controller_get_uint64(
                        pc_sampling_handle, kRPVControllerPCSamplingNumPcSampleStates, 0,
                        &sample_state_count) == kRocProfVisResultSuccess);
            REQUIRE(sample_state_count == 0);
        ...
        {
            spdlog::info("PC sampling is unsupported by this compute trace");
        }
