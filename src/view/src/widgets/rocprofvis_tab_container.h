...
public:
    ...
    // Fills in the entries of a tab's right-click menu. Called from inside the open
    // popup with the tab that was right-clicked. Tabs have no context menu while the
    // callback is unset.
    void SetTabContextMenuCallback(std::function<void(const TabItem&)> callback);

private:
    void                 ShowCloseTabConfirm(int tab_index);
    void                 RenderTabContextMenu();
    ...
    // Id of the tab whose context menu is open, empty while no menu is open. Tracked
    // by id rather than index so closing a tab cannot retarget an open menu.
    std::string                         m_context_menu_tab_id;
    std::function<void(const TabItem&)> m_tab_context_menu_callback;
