       ...
       timestamp <= end_timestamp)
    {
        ...
        if(segments.GetSegments().find(segment_start) == segments.GetSegments().end())
        {
            ...
            std::unique_ptr<Segment> segment     = std::make_unique<Segment>(
                (rocprofvis_controller_track_type_t) track_type, m_ctx);
            ...
            if (end > start)
            {
                ...
                if(fetch_ranges.size())
                {
                    for(auto& range : fetch_ranges)
                    {
                    	...
                    	args.m_lru_params.m_ctx      = m_track->GetContext();
                        ...
                        m_track->GetContext()->GetMemoryManager()->CancelArrayOwnersip(&args.m_entries);
                    }
                }
                else
                {
...
Graph::Graph(Trace* ctx, rocprofvis_controller_graph_type_t type, uint64_t id)
: m_id(id)
, m_track(nullptr)
, m_type(type)
, m_ctx(ctx)
...
{
    std::shared_lock<std::shared_mutex> lock(m_ctx->GetMemoryManager()->GetMemoryManagerMutex());
    rocprofvis_result_t result = kRocProfVisResultUnknownError;
