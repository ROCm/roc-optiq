// Copyright Advanced Micro Devices, Inc.
// SPDX-License-Identifier: MIT

#include "rocprofvis_c_interface.h"
#include "profiler_hub_lib_interface.h"
#include "rocprofvis_core.h"
#include "rocprofvis_db_future.h"
#include "rocprofvis_error_handling.h"
#include <algorithm>
#include <catch2/catch_session.hpp>
#include <catch2/catch_test_macros.hpp>
#include <cstdarg>
#include <cstdio>
#include <filesystem>
#include <string.h>
#include <vector>

#define MULTI_LINE_LOG_START auto multi_line_log = fmt::memory_buffer()
#define MULTI_LINE_LOG(format, ...)                                                      \
    fmt::format_to(std::back_inserter(multi_line_log), format, __VA_ARGS__)
#define MULTI_LINE_LOG_ARGS "{:.{}}", multi_line_log.data(), multi_line_log.size()

std::string g_input_file = "sample/trace_70b_1024_32.rpd";
bool        g_full_range = false;

void
ReadSliceData(rocprofvis_dm_trace_t trace, uint32_t num_tracks,
              rocprofvis_dm_timestamp_t start_time, rocprofvis_dm_timestamp_t end_time,
              uint32_t accessInMillisec, rocprofvis_db_future_t object2wait);
void
DeleteSliceData(rocprofvis_dm_trace_t trace, rocprofvis_dm_timestamp_t start_time,
                rocprofvis_dm_timestamp_t end_time, uint32_t accessInMillisec);

int
main(int argc, char** argv)
{
    Catch::Session session;

    using namespace Catch::Clara;
    auto cli = session.cli() |
               Opt(g_input_file, "input_file")["--input_file"]("Path to input file") |
               Opt(g_full_range, "full_range")["--full_range"](
                   "Whether to load/query the full trace range or only a segment");

    // Now pass the new composite back to Catch2 so it uses that
    session.cli(cli);

    // Let Catch2 (using Clara) parse the command line
    int returnCode = session.applyCommandLine(argc, argv);
    if(returnCode != 0)  // Indicates a command line error
        return returnCode;

    std::string log_file = "Testing/Temporary/rocprofvis_dm_system_tests/" +
                           std::filesystem::path(g_input_file).filename().string() +
                           ".txt";
    rocprofvis_core_enable_log(log_file.c_str(), spdlog::level::trace);

    return session.run();
}

void
CheckMemoryFootprint(rocprofvis_dm_trace_t trace)
{
    uint64_t memory_used = 0;
    if(trace)
    {
        memory_used += rocprofvis_dm_get_property_as_uint64(
            trace, kRPVDMTraceMemoryFootprintUInt64, 0);
        rocprofvis_dm_database_t db =
            rocprofvis_dm_get_property_as_handle(trace, kRPVDMDatabaseHandle, 0);
        if(db)
        {
            memory_used += rocprofvis_db_get_memory_footprint(db);
        }
    }

    spdlog::info("\x1b[0mTotal memory utilization so far = {}", memory_used);
}

#define LIST_SIZE_LIMIT 2
#define HEADER_LEN      100

void
PrintHeader(const char* fmt, ...)
{
    std::string header;
    va_list     argptr;
    char        buffer[256];
    va_start(argptr, fmt);
    vsnprintf(buffer, sizeof(buffer), fmt, argptr);
    va_end(argptr);
    size_t text_len = strlen(buffer);
    if(HEADER_LEN > text_len) header.assign((HEADER_LEN - text_len) / 2, '*');

    spdlog::info("\x1b[0m{0}{1}{2}", header, std::string(buffer), header);
}

void
progress_callback(rocprofvis_db_filename_t db_name, rocprofvis_db_future_id_t id,
            rocprofvis_progress_callback_percent_t progress, rocprofvis_db_status_t status,
            rocprofvis_db_status_message_t msg, void* user_data)
{
    (void) id;
    (void) user_data;
    const char* str   = " ERROR ";
    const char* color = "\x1b[31m";
    if(status == kRPVDbSuccess)
    {
        color = "\x1b[32m";
        str   = " DONE ";
    }
    else if(status == kRPVDbBusy)
    {
        color = "\x1b[33m";
        str   = " BUSY ";
    }

    spdlog::info("{0}[{1}] {2} - {3} - {4}", color, db_name, progress, str, msg);
}

void
GenerateRandomSlice(rocprofvis_dm_trace_t trace, rocprofvis_db_num_of_tracks_t& count,
                    rocprofvis_db_track_selection_t& tracks,
                    rocprofvis_dm_timestamp_t&       start_time,
                    rocprofvis_dm_timestamp_t&       end_time)
{
    static std::vector<uint32_t> v;
    v.clear();
    uint64_t num_tracks =
        rocprofvis_dm_get_property_as_uint64(trace, kRPVDMNumberOfTracksUInt64, 0);

    int rand_num_tracks = 1;
    for(int i = 0; i < rand_num_tracks; i++)
    {
        while(true)
        {
            uint32_t track_id1 = std::rand() % std::max(num_tracks, (uint64_t) 10);
            if(std::find_if(v.begin(), v.end(), [track_id1](uint32_t track_id2) {
                   return track_id2 == track_id1;
               }) == v.end())
            {
                v.push_back(track_id1);
                break;
            }
        }
    }
    count = (rocprofvis_db_num_of_tracks_t) rand_num_tracks;

    tracks                               = &v[0];
    rocprofvis_dm_timestamp_t tenth_time = (end_time - start_time) / 10;
    int                       pie1       = std::rand() % 10;
    int                       pie2       = std::rand() % 10;
    if((pie1 + pie2) > 10) pie2 = 10 - pie1;
    if(!g_full_range)
    {
        start_time = start_time + pie1 * tenth_time;
        end_time   = start_time + pie2 * tenth_time;
    }
    spdlog::info("Testing slice for {} ns and {} tracks", end_time - start_time,
                 (uint32_t) count);
    spdlog::info("Track indexes: [");
    for(int i = 0; i < count; i++)
        spdlog::info("{}", tracks[i]);
    spdlog::info("]");
}

void
ReadSliceData(rocprofvis_dm_trace_t trace, uint32_t num_tracks,
              rocprofvis_dm_timestamp_t start_time, rocprofvis_dm_timestamp_t end_time,
              uint32_t accessInMillisec, rocprofvis_db_future_t object2wait)
{
    uint32_t num_rows = 0;
    if(object2wait != nullptr)
    {
        for(uint32_t i = 0; i < accessInMillisec; i++)
        {
            num_rows =
                ((RocProfVis::DataModel::Future*) object2wait)->GetProcessedRowsCount();
            if(num_rows > 1000)
            {
                break;
            }
            std::this_thread::sleep_for(std::chrono::microseconds(1000));
        }
        spdlog::info(ANSI_COLOR_MAGENTA
                     "{0} rows processed or {1} milliseconds passed. Start "
                     "accessing data" ANSI_COLOR_RESET,
                     num_rows, accessInMillisec);
    }
    else
    {
        std::this_thread::sleep_for(std::chrono::milliseconds(accessInMillisec));
        spdlog::info(ANSI_COLOR_MAGENTA "{0} milliseconds passed. Start "
                                        "accessing data" ANSI_COLOR_RESET,
                     accessInMillisec);
    }

    for(uint32_t i = 0; i < num_tracks; i++)
    {
        rocprofvis_dm_track_t track =
            rocprofvis_dm_get_property_as_handle(trace, kRPVDMTrackHandleIndexed, i);
        REQUIRE(track != nullptr);
        uint64_t              hash_time = rocprofvis_dm_hash_combine_timestamp(start_time, end_time, kRocProfVisDmHashedTimestampTagTrackSlice);
        rocprofvis_dm_slice_t slice     = rocprofvis_dm_get_property_as_handle(
            track, kRPVDMSliceHandleTimed, hash_time);
        if(slice != nullptr)
        {
            uint64_t num_records = rocprofvis_dm_get_property_as_uint64(
                slice, kRPVDMNumberOfRecordsUInt64, 0);
            spdlog::info(ANSI_COLOR_MAGENTA
                         "Track({0}) Slice({1}) has {2} records" ANSI_COLOR_RESET,
                         track, start_time, num_records);
        }
    }
}

void
DeleteSliceData(rocprofvis_dm_trace_t trace, rocprofvis_dm_timestamp_t start_time,
                rocprofvis_dm_timestamp_t end_time, uint32_t accessInMillisec)
{
    std::this_thread::sleep_for(std::chrono::milliseconds(accessInMillisec));
    spdlog::info(ANSI_COLOR_MAGENTA "{0} milliseconds passed. Start "
                                    "deleting slice({1})" ANSI_COLOR_RESET,
                 accessInMillisec, start_time);
    rocprofvis_dm_delete_time_slice(trace, start_time, end_time);
}

void ReportTraceType(rocprofvis_db_type_t db_type)
{
    spdlog::info(ANSI_COLOR_CYAN"Database type : {}",
        db_type == kAutodetect ? "cannot be detected" :
        db_type == kProfilerHub ? "profiler hub supported" :
        db_type == kRocpdSqlite ? "legacy rocpd" :
        db_type == kRocprofSqlite ? "modern rocpd" :
        db_type == kRocprofMultinodeSqlite ? "multi-node rocpd" :
        db_type == kComputeSqlite ? "compute database" :
        db_type == kChromeTrace ? "chrome trace" :
        db_type == kPerfettoTrace ? "perfetto trace" :
        db_type == kGoogleSqlite ? "perfetto sqlite" :
        "unknown");
}

struct RocProfVisDMFixture
{
    mutable rocprofvis_dm_trace_t           m_trace = nullptr;
    mutable rocprofvis_dm_database_t        m_dm_db    = nullptr;
    mutable rocprofvis_dm_database_t        m_ph_db    = nullptr;
    mutable rocprofvis_dm_timestamp_t       m_start_time;
    mutable rocprofvis_dm_timestamp_t       m_end_time;
    mutable rocprofvis_db_num_of_tracks_t   m_num_tracks;
    mutable rocprofvis_db_track_selection_t m_tracks_selection;
    mutable rocprofvis_db_type_t            m_dm_db_type;
    mutable rocprofvis_db_type_t            m_ph_db_type;
};


TEST_CASE_PERSISTENT_FIXTURE(RocProfVisDMFixture, "System Trace Data-Model Tests")
{


    // Allocates a new DM trace handle.
    // Fixture Writes: m_trace
    SECTION("Create Trace")
    {
        PrintHeader("Create trace");
        m_trace = rocprofvis_dm_create_trace();
        REQUIRE(nullptr != m_trace);
    }

    SECTION("Detection using data-model")
    {
        PrintHeader("Detect database %s", g_input_file.c_str());
        m_dm_db_type = rocprofvis_db_identify_type(g_input_file.c_str());
        REQUIRE(kAutodetect != m_dm_db_type);
        ReportTraceType(m_ph_db_type);
    }
    SECTION("Detection using profiler-hub")
    {
        PrintHeader("Detect database %s", g_input_file.c_str());
        m_ph_db_type = (rocprofvis_db_type_t)profiler_hub_db_identify_type(g_input_file.c_str());
        REQUIRE(kAutodetect != m_ph_db_type);
        ReportTraceType(m_ph_db_type);
    }

    // Opens the database file and binds it to the trace handle.
    // Fixture Reads:  m_trace
    // Fixture Writes: m_db
    SECTION("Data-model trace initialisation")
    {
        CheckMemoryFootprint(m_trace);
        PrintHeader("Open database %s", g_input_file.c_str());
        m_dm_db = rocprofvis_db_open_database(g_input_file.c_str(), m_dm_db_type);
        REQUIRE(nullptr != m_dm_db);
        rocprofvis_dm_result_t bind_result =
            rocprofvis_dm_bind_trace_to_database(m_trace, m_dm_db, nullptr);
        REQUIRE(kRocProfVisDmResultSuccess == bind_result);
    }

    SECTION("Profiler-hub trace initialisation")
    {
        CheckMemoryFootprint(m_trace);
        PrintHeader("Open database %s", g_input_file.c_str());
        m_ph_db = rocprofvis_db_open_database(g_input_file.c_str(), m_ph_db_type);
        REQUIRE(nullptr != m_ph_db);
        rocprofvis_dm_result_t bind_result =
            rocprofvis_dm_bind_trace_to_database(m_trace, m_ph_db, nullptr);
        REQUIRE(kRocProfVisDmResultSuccess == bind_result);
    }

    // Loads database metadata (schema, indexes) asynchronously and waits for completion.
    // Fixture Reads: m_db
    SECTION("Read Metadata using data-model")
    {
        PrintHeader("Read Metadata using data-model");
        rocprofvis_db_future_t object2wait = rocprofvis_db_future_alloc(progress_callback);
        REQUIRE(object2wait);
        spdlog::info("Issue metadata read");
        rocprofvis_dm_result_t meta_result =
            rocprofvis_db_read_metadata_async(m_dm_db, object2wait);
        REQUIRE(kRocProfVisDmResultSuccess == meta_result);
        spdlog::info("Wait for metadata");
        rocprofvis_dm_result_t wait_result =
            rocprofvis_db_future_wait(object2wait, UINT64_MAX);
        REQUIRE(kRocProfVisDmResultSuccess == wait_result);
        rocprofvis_db_future_free(object2wait);
    }

    SECTION("Read Metadata using profiler-hub")
    {
        PrintHeader("Read Metadata using data-model");
        profiler_hub_future_handle_t object2wait = profiler_hub_future_alloc(progress_callback);
        REQUIRE(object2wait);
        spdlog::info("Issue metadata read");
        rocprofvis_dm_result_t meta_result =
            rocprofvis_db_read_metadata_async(m_ph_db, object2wait);
        REQUIRE(kRocProfVisDmResultSuccess == meta_result);
        spdlog::info("Wait for metadata");
        rocprofvis_dm_result_t wait_result =
            rocprofvis_db_future_wait(object2wait, UINT64_MAX);
        REQUIRE(kRocProfVisDmResultSuccess == wait_result);
        rocprofvis_db_future_free(object2wait);
    }

    // Reads per-track metadata including identity, category, process names,
    // record counts, time bounds, value range, and instance/process/subprocess IDs.
    // Fixture Reads: m_trace
    // Fixture Writes: m_start_time, m_end_time, m_num_tracks
    SECTION("Read Track Metadata")
    {
        CheckMemoryFootprint(m_trace);

        m_start_time =
            rocprofvis_dm_get_property_as_uint64(m_trace, kRPVDMStartTimeUInt64, 0);
        m_end_time =
            rocprofvis_dm_get_property_as_uint64(m_trace, kRPVDMEndTimeUInt64, 0);
        spdlog::info(ANSI_COLOR_GREEN "Trace start time={0}, end time = {1}",
                     m_start_time, m_end_time);
        m_num_tracks =
            (rocprofvis_db_num_of_tracks_t) rocprofvis_dm_get_property_as_uint64(
                m_trace, kRPVDMNumberOfTracksUInt64, 0);
        for(int i = 0; i < m_num_tracks; i++)
        {
            rocprofvis_dm_track_t track = rocprofvis_dm_get_property_as_handle(
                m_trace, kRPVDMTrackHandleIndexed, i);
            REQUIRE(track != nullptr);

            char const* category = rocprofvis_dm_get_property_as_charptr(
                track, kRPVDMTrackCategoryEnumCharPtr, 0);
            char const* process = rocprofvis_dm_get_property_as_charptr(
                track, kRPVDMTrackMainProcessNameCharPtr, 0);
            char const* subname = rocprofvis_dm_get_property_as_charptr(
                track, kRPVDMTrackSubProcessNameCharPtr, 0);
            REQUIRE(category);
            REQUIRE(process);
            REQUIRE(subname);

            uint64_t track_id =
                rocprofvis_dm_get_property_as_uint64(track, kRPVDMTrackIdUInt64, 0);
            uint64_t node_id =
                rocprofvis_dm_get_property_as_uint64(track, kRPVDMTrackNodeIdUInt64, 0);

            uint64_t num_records = rocprofvis_dm_get_property_as_uint64(
                track, kRPVDMTrackNumRecordsUInt64, 0);
            uint64_t min_timestamp = rocprofvis_dm_get_property_as_uint64(
                track, kRPVDMTrackMinimumTimestampUInt64, 0);
            uint64_t max_timestamp = rocprofvis_dm_get_property_as_uint64(
                track, kRPVDMTrackMaximumTimestampUInt64, 0);
            double min_value = rocprofvis_dm_get_property_as_double(
                track, kRPVDMTrackMinimumValueDouble, 0);
            double max_value = rocprofvis_dm_get_property_as_double(
                track, kRPVDMTrackMaximumValueDouble, 0);

            uint64_t instance_id = rocprofvis_dm_get_property_as_uint64(
                track, kRPVDMTrackInstanceIdUInt64, 0);
            uint64_t process_id = rocprofvis_dm_get_property_as_uint64(
                track, kRPVDMTrackProcessIdUInt64, 0);
            uint64_t subprocess_id = rocprofvis_dm_get_property_as_uint64(
                track, kRPVDMTrackSubProcessIdUInt64, 0);

            spdlog::info(ANSI_COLOR_CYAN
                         "Track id={0} node={1} category={2} process={3} subprocess={4} "
                         "records={5} time=[{6},{7}] value=[{8},{9}] "
                         "instance={10} pid={11} subpid={12}",
                         track_id, node_id, category, process, subname, num_records,
                         min_timestamp, max_timestamp, min_value, max_value, instance_id,
                         process_id, subprocess_id);
        }
    }

    // Retrieves the topology root from the trace and recursively walks all nodes,
    // reading name, type, optional track binding, keyed properties, and children.
    // Fixture Reads: m_trace
    SECTION("Read Topology")
    {
        CheckMemoryFootprint(m_trace);
        PrintHeader("Read topology tree");

        rocprofvis_dm_topology_node root =
            rocprofvis_dm_get_property_as_handle(m_trace, kRPVDMTopologyHandle, 0);
        REQUIRE(root != nullptr);

        struct NodeVisitor
        {
            static void Visit(rocprofvis_dm_topology_node node, int depth)
            {
                char* name = rocprofvis_dm_get_property_as_charptr(
                    node, kRPVControllerTopologyNodeName, 0);
                uint64_t type = rocprofvis_dm_get_property_as_uint64(
                    node, kRPVControllerTopologyNodeType, 0);

                std::string indent(depth * 2, ' ');
                spdlog::info("{}Node: name={}, type={}", indent, name ? name : "(null)",
                             type);

                rocprofvis_dm_get_property_as_uint64(node,
                                                     kRPVControllerTopologyNodeTrack, 0);

                uint64_t num_properties = rocprofvis_dm_get_property_as_uint64(
                    node, kRPVControllerTopologyNodeNumProperties, 0);
                for(uint64_t p = 0; p < num_properties; p++)
                {
                    uint64_t key = rocprofvis_dm_get_property_as_uint64(
                        node, kRPVControllerTopologyNodePropertyKeyIndexed, p);
                    uint64_t prop_type = rocprofvis_dm_get_property_as_uint64(
                        node, kRPVControllerTopologyNodePropertyTypeIndexed, p);
                    if(prop_type == kRPVDataTypeString)
                    {
                        char* str_value = rocprofvis_dm_get_property_as_charptr(
                            node, kRPVControllerTopologyNodePropertyValueIndexed, p);
                        spdlog::trace("{}  Property {}: key={}, value={}, type=string",
                                      indent, p, key, str_value ? str_value : "(null)");
                    }
                    else if(prop_type == kRPVDataTypeDouble)
                    {
                        double dbl_value = rocprofvis_dm_get_property_as_double(
                            node, kRPVControllerTopologyNodePropertyValueIndexed, p);
                        spdlog::trace("{}  Property {}: key={}, value={}, type=double",
                                      indent, p, key, dbl_value);
                    }
                    else
                    {
                        uint64_t int_value = rocprofvis_dm_get_property_as_uint64(
                            node, kRPVControllerTopologyNodePropertyValueIndexed, p);
                        spdlog::trace("{}  Property {}: key={}, value={}, type=int",
                                      indent, p, key, int_value);
                    }
                }

                uint64_t num_children = rocprofvis_dm_get_property_as_uint64(
                    node, kRPVControllerTopologyNodeNumChildren, 0);
                for(uint64_t c = 0; c < num_children; c++)
                {
                    rocprofvis_dm_topology_node child =
                        rocprofvis_dm_get_property_as_handle(
                            node, kRPVControllerTopologyNodeChildHandleIndexed, c);
                    REQUIRE(child != nullptr);
                    Visit(child, depth + 1);
                }
            }
        };

        NodeVisitor::Visit(root, 0);
    }

    // Reads global histogram bucket dimensions from the trace, then iterates a
    // subset of tracks reading per-track bucket event density and bucket value.
    // Fixture Reads: m_trace, m_num_tracks
    SECTION("Read Histogram Data")
    {
        CheckMemoryFootprint(m_trace);
        PrintHeader("Read histogram dimensions");

        uint64_t num_buckets =
            rocprofvis_dm_get_property_as_uint64(m_trace, kRPVDMHistogramNumBuckets, 0);
        uint64_t bucket_size =
            rocprofvis_dm_get_property_as_uint64(m_trace, kRPVDMHistogramBucketSize, 0);
        spdlog::info(ANSI_COLOR_GREEN "Histogram: num_buckets={}, bucket_size={}",
                     num_buckets, bucket_size);

        if(num_buckets > 0)
        {
            int tracks_to_check = std::min((int) m_num_tracks, LIST_SIZE_LIMIT);
            for(int i = 0; i < tracks_to_check; i++)
            {
                rocprofvis_dm_track_t track = rocprofvis_dm_get_property_as_handle(
                    m_trace, kRPVDMTrackHandleIndexed, i);
                REQUIRE(track != nullptr);

                char const* category = rocprofvis_dm_get_property_as_charptr(
                    track, kRPVDMTrackCategoryEnumCharPtr, 0);
                spdlog::trace("Track {} ({}) histogram buckets:", i,
                              category ? category : "(null)");

                for(uint64_t b = 0; b < num_buckets; b++)
                {
                    uint64_t density = rocprofvis_dm_get_property_as_uint64(
                        track, kRPVDMTrackHistogramBucketEventDensityUInt64Indexed, b);
                    double bucket_value = rocprofvis_dm_get_property_as_double(
                        track, kRPVDMTrackHistogramBucketValueDoubleIndexed, b);
                    spdlog::trace("  bucket[{}]: density={}, value={}", b, density,
                                  bucket_value);
                }
            }
        }
    }

    // Saves a trimmed time range of the trace to a temporary file, validates
    // success, then removes the file.
    // Fixture Reads:  m_trace, m_db, m_start_time, m_end_time
    // Fixture Writes: m_num_tracks, m_tracks_selection
    SECTION("Trim Trace")
    {
        PrintHeader("Trim trace");
        rocprofvis_db_future_t object2wait = rocprofvis_db_future_alloc(progress_callback);
        REQUIRE(nullptr != object2wait);

        rocprofvis_dm_timestamp_t start_time = m_start_time;
        rocprofvis_dm_timestamp_t end_time   = m_end_time;
        GenerateRandomSlice(m_trace, m_num_tracks, m_tracks_selection, start_time,
                            end_time);
        rocprofvis_dm_result_t result = rocprofvis_db_trim_save_async(
            m_db, start_time, (start_time + ((end_time - start_time) / 2)),
            "sample/trimmed.db", object2wait);
        REQUIRE(result == kRocProfVisDmResultSuccess);

        rocprofvis_dm_result_t read_wait =
            rocprofvis_db_future_wait(object2wait, UINT64_MAX);
        REQUIRE(kRocProfVisDmResultSuccess == read_wait);
        std::filesystem::remove("sample/trimmed.db");

        rocprofvis_db_future_free(object2wait);
    }

    // Reads each selected track's full time range one at a time, measuring read
    // times and row counts per track and in total.
    // Fixture Reads:  m_trace, m_db, m_start_time, m_end_time, m_num_tracks
    // Fixture Writes: m_num_tracks, m_tracks_selection
    SECTION("Whole Trace Read")
    {
        PrintHeader("Whole Trace Read");
        double   whole_trace_readtime = 0;
        uint32_t total_num_rows       = 0;

        rocprofvis_dm_timestamp_t start_time = m_start_time;
        rocprofvis_dm_timestamp_t end_time   = m_end_time;
        GenerateRandomSlice(m_trace, m_num_tracks, m_tracks_selection, start_time,
                            end_time);
        for(int i = 0; i < m_num_tracks; i++)
        {
            rocprofvis_dm_track_t track = rocprofvis_dm_get_property_as_handle(
                m_trace, kRPVDMTrackHandleIndexed, i);
            REQUIRE(track != nullptr);

            bool load_slice = false;
            for(int j = 0; j < m_num_tracks; j++)
            {
                if(m_tracks_selection[j] == static_cast<uint32_t>(i))
                {
                    load_slice = true;
                    break;
                }
            }
            if(load_slice)
            {
                rocprofvis_db_future_t object2wait =
                    rocprofvis_db_future_alloc(progress_callback);
                REQUIRE(nullptr != object2wait);

                auto                   t1 = std::chrono::steady_clock::now();
                rocprofvis_dm_result_t read_slice_issue =
                    rocprofvis_db_read_trace_slice_async(m_db, start_time, end_time,
                                                         kRocProfVisDmHashedTimestampTagTrackSlice, 1,
                                                         (uint32_t*) &i, object2wait);
                REQUIRE(kRocProfVisDmResultSuccess == read_slice_issue);
                if(kRocProfVisDmResultSuccess == read_slice_issue)
                {
                    rocprofvis_dm_result_t read_wait =
                        rocprofvis_db_future_wait(object2wait, UINT64_MAX);
                    REQUIRE(kRocProfVisDmResultSuccess == read_wait);
                    if(kRocProfVisDmResultSuccess == read_wait)
                    {
                        auto t2 = std::chrono::steady_clock::now();
                        std::chrono::duration<double> diff = t2 - t1;
                        whole_trace_readtime += diff.count();
                        uint32_t num_rows = ((RocProfVis::DataModel::Future*) object2wait)
                                                ->GetProcessedRowsCount();
                        total_num_rows += num_rows;
                        uint64_t hash_time = rocprofvis_dm_hash_combine_timestamp(start_time, end_time, kRocProfVisDmHashedTimestampTagTrackSlice);
                        rocprofvis_dm_slice_t slice =
                            rocprofvis_dm_get_property_as_handle(
                                track, kRPVDMSliceHandleTimed, hash_time);
                        REQUIRE(slice);
                        uint64_t num_records = rocprofvis_dm_get_property_as_uint64(
                            slice, kRPVDMNumberOfRecordsUInt64, 0);
                        spdlog::info(
                            ANSI_COLOR_MAGENTA
                            "Track {0} has {1} records, read time - {2:13.9f}, number of "
                            "rows processed = {3}" ANSI_COLOR_RESET,
                            i, num_records, diff.count(), num_rows);
                        rocprofvis_dm_delete_all_time_slices(m_trace);
                    }
                }

                rocprofvis_db_future_free(object2wait);
            }
        }

        spdlog::info(ANSI_COLOR_MAGENTA
                     "Whole trace read time - {0:13.9f},  number of rows "
                     "processed = {1}" ANSI_COLOR_RESET,
                     whole_trace_readtime, total_num_rows);
    }

    // Reads a random time slice for selected tracks, validates record access
    // including event properties, flow traces, stack traces, extended data,
    // argument records, and PMC sample values.
    // Fixture Reads: m_trace, m_db, m_start_time, m_end_time, m_num_tracks,
    //        m_tracks_selection
    SECTION("Read Random Slice Data")
    {
        PrintHeader("Read Random Slice Data");
        if(m_num_tracks > 0)
        {
            rocprofvis_db_future_t object2wait = rocprofvis_db_future_alloc(progress_callback);
            REQUIRE(nullptr != object2wait);

            spdlog::info("Read time slice for all tracks");
            auto                   t1 = std::chrono::steady_clock::now();
            rocprofvis_dm_result_t read_slice_result =
                rocprofvis_db_read_trace_slice_async(m_db, m_start_time, m_end_time,
                                                     kRocProfVisDmHashedTimestampTagTrackSlice,
                                                     m_num_tracks, m_tracks_selection,
                                                     object2wait);
            REQUIRE(kRocProfVisDmResultSuccess == read_slice_result);
            rocprofvis_dm_result_t slice_wait_result =
                rocprofvis_db_future_wait(object2wait, UINT64_MAX);
            REQUIRE(kRocProfVisDmResultSuccess == slice_wait_result);
            auto                          t2   = std::chrono::steady_clock::now();
            std::chrono::duration<double> diff = t2 - t1;
            uint32_t                      num_rows =
                ((RocProfVis::DataModel::Future*) object2wait)->GetProcessedRowsCount();
            spdlog::info(ANSI_COLOR_MAGENTA
                         "Whole trace read time - {0:13.9f}, number of "
                         "rows processed = {1}" ANSI_COLOR_RESET,
                         diff.count(), num_rows);
            CheckMemoryFootprint(m_trace);
            spdlog::info("Time slice content, up to {} records", LIST_SIZE_LIMIT);
            int first_track = std::rand() % m_num_tracks;
            for(int i = first_track;
                (i < m_num_tracks) && (i < first_track + LIST_SIZE_LIMIT); i++)
            {
                rocprofvis_dm_track_t track = rocprofvis_dm_get_property_as_handle(
                    m_trace, kRPVDMTrackHandleIndexed, m_tracks_selection[i]);
                REQUIRE(track != nullptr);
                char* track_category_name = rocprofvis_dm_get_property_as_charptr(
                    track, kRPVDMTrackCategoryEnumCharPtr, 0);
                char* track_process_name = rocprofvis_dm_get_property_as_charptr(
                    track, kRPVDMTrackMainProcessNameCharPtr, 0);
                char* track_sub_process_name = rocprofvis_dm_get_property_as_charptr(
                    track, kRPVDMTrackSubProcessNameCharPtr, 0);
                REQUIRE(track_category_name);
                REQUIRE(track_process_name);
                REQUIRE(track_sub_process_name);
                rocprofvis_dm_track_category_t track_category =
                    (rocprofvis_dm_track_category_t) rocprofvis_dm_get_property_as_uint64(
                        track, kRPVDMTrackCategoryEnumUInt64, 0);
                uint64_t              hash_time = rocprofvis_dm_hash_combine_timestamp(m_start_time, m_end_time, kRocProfVisDmHashedTimestampTagTrackSlice);
                rocprofvis_dm_slice_t slice     = rocprofvis_dm_get_property_as_handle(
                    track, kRPVDMSliceHandleTimed, hash_time);
                REQUIRE(slice != nullptr);
                uint64_t track_id =
                    rocprofvis_dm_get_property_as_uint64(track, kRPVDMTrackIdUInt64, 0);
                uint64_t node_id = rocprofvis_dm_get_property_as_uint64(
                    track, kRPVDMTrackNodeIdUInt64, 0);
                uint64_t memory_usage = rocprofvis_dm_get_property_as_uint64(
                    track, kRPVDMTrackMemoryFootprintUInt64, 0);
                uint64_t num_ext_data = rocprofvis_dm_get_property_as_uint64(
                    track, kRPVDMNumberOfTrackExtDataRecordsUInt64, 0);
                spdlog::info("Track id={} node={} category={} process={} "
                             "subprocess={}",
                             track_id, node_id, track_category_name, track_process_name,
                             track_sub_process_name);

                spdlog::info(ANSI_COLOR_CYAN "\t{0} : {1} : {2}", "Properties",
                             "Memory usage", memory_usage);
                for(int ext_data_index = 0; ext_data_index < num_ext_data; ext_data_index++)
                {
                    char* ext_data_category = rocprofvis_dm_get_property_as_charptr(
                        track, kRPVDMTrackExtDataCategoryCharPtrIndexed, ext_data_index);
                    char* ext_data_name = rocprofvis_dm_get_property_as_charptr(
                        track, kRPVDMTrackExtDataNameCharPtrIndexed, ext_data_index);
                    char* ext_data_value = rocprofvis_dm_get_property_as_charptr(
                        track, kRPVDMTrackExtDataValueCharPtrIndexed, ext_data_index);
                    REQUIRE(ext_data_category);
                    REQUIRE(ext_data_name);
                    REQUIRE(ext_data_value);
                    spdlog::info(ANSI_COLOR_CYAN "\t{0} : {1} : {2}", ext_data_category,
                                 ext_data_name, ext_data_value);
                }
                if(nullptr != slice)
                {
                    uint64_t num_records = rocprofvis_dm_get_property_as_uint64(
                        slice, kRPVDMNumberOfRecordsUInt64, 0);
                    spdlog::info(ANSI_COLOR_BLUE
                                 "Time slice for time {0} - {1} for "
                                 "track {2} [{3}:{4}:{5}] has {6} records",
                                 m_start_time, m_end_time, m_tracks_selection[i],
                                 track_category_name, track_process_name,
                                 track_sub_process_name, num_records);
                    if(num_records == 0) continue;
                    int first_record = std::rand() % num_records;
                    for(int j = first_record;
                        (j < num_records) && (j < first_record + LIST_SIZE_LIMIT); j++)
                    {
                        uint64_t timestamp = rocprofvis_dm_get_property_as_uint64(
                            slice, kRPVDMTimestampUInt64Indexed, j);
                        if(track_category ==
                               rocprofvis_dm_track_category_t::kRocProfVisDmRegionTrack ||
                           track_category == rocprofvis_dm_track_category_t::
                                                 kRocProfVisDmKernelDispatchTrack ||
                           track_category == rocprofvis_dm_track_category_t::
                                                 kRocProfVisDmMemoryAllocationTrack ||
                           track_category == rocprofvis_dm_track_category_t::
                                                 kRocProfVisDmMemoryCopyTrack)

                        {
                            int64_t duration = rocprofvis_dm_get_property_as_int64(
                                slice, kRPVDMEventDurationInt64Indexed, j);
                            if(duration < 0)
                            {
                                spdlog::info(ANSI_COLOR_RED
                                             "Record {0} has invalid duration "
                                             "{1}" ANSI_COLOR_RESET,
                                             j, duration);
                            }
                            uint64_t event_id = rocprofvis_dm_get_property_as_uint64(
                                slice, kRPVDMEventIdUInt64Indexed, j);
                            uint64_t op = rocprofvis_dm_get_property_as_uint64(
                                slice, kRPVDMEventIdOperationEnumIndexed, j);
                            char* op_str = rocprofvis_dm_get_property_as_charptr(
                                slice, kRPVDMEventIdOperationCharPtrIndexed, j);
                            char* type_str = rocprofvis_dm_get_property_as_charptr(
                                slice, kRPVDMEventTypeStringCharPtrIndexed, j);
                            char* symbol_str = rocprofvis_dm_get_property_as_charptr(
                                slice, kRPVDMEventSymbolStringCharPtrIndexed, j);
                            REQUIRE(op_str);
                            REQUIRE(type_str);
                            REQUIRE(symbol_str);
                            uint64_t event_level = rocprofvis_dm_get_property_as_uint64(
                                slice, kRPVDMEventLevelUInt64Indexed, j);
                            spdlog::info(ANSI_COLOR_BLUE
                                         "Record id={0}, timestamp={1}, op={2}, "
                                         "op_str={3}, type={4}, symbol={5}, "
                                         "level={6}",
                                         event_id, timestamp, op, op_str, type_str,
                                         symbol_str, event_level);

                            spdlog::info("Testing multithreaded access to "
                                         "database retrieving flow, stack and "
                                         "extended data for event id = {}",
                                         event_id);
                            rocprofvis_dm_event_id_t dm_event_id;
                            dm_event_id.value = event_id;
                            rocprofvis_db_future_t object2wait4flowtrace =
                                rocprofvis_db_future_alloc(progress_callback);
                            REQUIRE(object2wait4flowtrace);
                            rocprofvis_db_future_t object2wait4stacktrace =
                                rocprofvis_db_future_alloc(progress_callback);
                            REQUIRE(object2wait4stacktrace);
                            rocprofvis_db_future_t object2wait4extdata =
                                rocprofvis_db_future_alloc(progress_callback);
                            REQUIRE(object2wait4extdata);
                            rocprofvis_dm_result_t result4flowtrace =
                                rocprofvis_db_read_event_property_async(
                                    m_db, kRPVDMEventFlowTrace, dm_event_id,
                                    object2wait4flowtrace);
                            REQUIRE(kRocProfVisDmResultSuccess == result4flowtrace);
                            rocprofvis_dm_result_t result4stacktrace =
                                rocprofvis_db_read_event_property_async(
                                    m_db, kRPVDMEventStackTrace, dm_event_id,
                                    object2wait4stacktrace);
                            REQUIRE(kRocProfVisDmResultSuccess == result4stacktrace);
                            rocprofvis_dm_result_t result4extdata =
                                rocprofvis_db_read_event_property_async(
                                    m_db, kRPVDMEventExtData, dm_event_id,
                                    object2wait4extdata);
                            REQUIRE(kRocProfVisDmResultSuccess == result4extdata);

                            rocprofvis_dm_result_t result4flowtrace_wait =
                                rocprofvis_db_future_wait(object2wait4flowtrace,
                                                          UINT64_MAX);
                            REQUIRE(kRocProfVisDmResultSuccess == result4flowtrace_wait);

                            if(kRocProfVisDmResultSuccess == result4flowtrace &&
                               kRocProfVisDmResultSuccess == result4flowtrace_wait)
                            {
                                rocprofvis_dm_flowtrace_t flowtrace =
                                    rocprofvis_dm_get_property_as_handle(
                                        m_trace, kRPVDMFlowTraceHandleByEventID,
                                        dm_event_id.value);
                                REQUIRE(flowtrace != nullptr);
                                spdlog::info("Data flow trace for event id = {}",
                                             (uint64_t) dm_event_id.bitfield.event_id);
                                uint64_t num_endpoints =
                                    rocprofvis_dm_get_property_as_uint64(
                                        flowtrace, kRPVDMNumberOfEndpointsUInt64, 0);
                                spdlog::info(ANSI_COLOR_MAGENTA
                                             "Event has {0} data flow link:",
                                             num_endpoints);
                                for(int k = 0; k < num_endpoints; k++)
                                {
                                    uint64_t endpoint_track_id =
                                        rocprofvis_dm_get_property_as_uint64(
                                            flowtrace, kRPVDMEndpointTrackIDUInt64Indexed,
                                            k);
                                    rocprofvis_dm_event_id_t endpoint_event_id;
                                    endpoint_event_id.value =
                                        rocprofvis_dm_get_property_as_uint64(
                                            flowtrace, kRPVDMEndpointIDUInt64Indexed, k);
                                    uint64_t event_timestamp =
                                        rocprofvis_dm_get_property_as_uint64(
                                            flowtrace,
                                            kRPVDMEndpointTimestampUInt64Indexed, k);
                                    uint64_t end_timestamp =
                                        rocprofvis_dm_get_property_as_uint64(
                                            flowtrace,
                                            kRPVDMEndpointEndTimestampUInt64Indexed, k);
                                    char* endpoint_category =
                                        rocprofvis_dm_get_property_as_charptr(
                                            flowtrace,
                                            kRPVDMEndpointCategoryCharPtrIndexed, k);
                                    char* endpoint_symbol =
                                        rocprofvis_dm_get_property_as_charptr(
                                            flowtrace, kRPVDMEndpointSymbolCharPtrIndexed,
                                            k);
                                    uint64_t endpoint_level =
                                        rocprofvis_dm_get_property_as_uint64(
                                            flowtrace, kRPVDMEndpointLevelUInt64Indexed,
                                            k);
                                    uint64_t event_id_num =
                                        endpoint_event_id.bitfield.event_id;
                                    spdlog::info(
                                        ANSI_COLOR_MAGENTA "\tEndpoint {0} at track={1}, "
                                                           "event_id={2}, timestamp={3}, "
                                                           "end_ts={4}, category={5}, "
                                                           "symbol={6}, level={7}",
                                        k, endpoint_track_id, event_id_num,
                                        event_timestamp, end_timestamp,
                                        endpoint_category ? endpoint_category : "(null)",
                                        endpoint_symbol ? endpoint_symbol : "(null)",
                                        endpoint_level);
                                }
                                rocprofvis_dm_delete_event_property_for(
                                    m_trace, kRPVDMEventFlowTrace, dm_event_id);
                            }
                            rocprofvis_db_future_free(object2wait4flowtrace);

                            rocprofvis_dm_result_t stacktrack_wait =
                                rocprofvis_db_future_wait(object2wait4stacktrace,
                                                          UINT64_MAX);
                            REQUIRE(kRocProfVisDmResultSuccess == stacktrack_wait);
                            if(kRocProfVisDmResultSuccess == result4stacktrace &&
                               kRocProfVisDmResultSuccess == stacktrack_wait)
                            {
                                rocprofvis_dm_stacktrace_t stacktrace =
                                    rocprofvis_dm_get_property_as_handle(
                                        m_trace, kRPVDMStackTraceHandleByEventID,
                                        dm_event_id.value);
                                REQUIRE(stacktrace != nullptr);
                                spdlog::info("Stack trace for event id = {}",
                                             (uint64_t) dm_event_id.bitfield.event_id);
                                uint64_t num_frames =
                                    rocprofvis_dm_get_property_as_uint64(
                                        stacktrace, kRPVDMNumberOfFramesUInt64, 0);
                                spdlog::info(ANSI_COLOR_MAGENTA
                                             "Event has {} stack frames:",
                                             num_frames);
                                for(int k = 0; k < num_frames; k++)
                                {
                                    uint64_t frame_region_id =
                                        rocprofvis_dm_get_property_as_uint64(
                                            stacktrace, kRPVDMFrameRegionIdUInt64Indexed,
                                            k);
                                    uint64_t frame_depth =
                                        rocprofvis_dm_get_property_as_uint64(
                                            stacktrace, kRPVDMFrameDepthUInt64Indexed, k);
                                    char* frame_symbol =
                                        rocprofvis_dm_get_property_as_charptr(
                                            stacktrace, kRPVDMFrameSymbolCharPtrIndexed,
                                            k);
                                    char* frame_args =
                                        rocprofvis_dm_get_property_as_charptr(
                                            stacktrace, kRPVDMFrameArgsCharPtrIndexed, k);
                                    char* frame_code =
                                        rocprofvis_dm_get_property_as_charptr(
                                            stacktrace, kRPVDMFrameCodeLineCharPtrIndexed,
                                            k);
                                    REQUIRE(frame_symbol);
                                    REQUIRE(frame_args);
                                    REQUIRE(frame_code);
                                    spdlog::info(ANSI_COLOR_MAGENTA
                                                 "\tFrame {0} : region_id={1}, "
                                                 "depth={2}, symbol={3}, args={4}, "
                                                 "code={5} \n",
                                                 k, frame_region_id, frame_depth,
                                                 frame_symbol, frame_args, frame_code);
                                }
                                rocprofvis_dm_delete_event_property_for(
                                    m_trace, kRPVDMEventStackTrace, dm_event_id);
                            }
                            rocprofvis_db_future_free(object2wait4stacktrace);

                            rocprofvis_dm_result_t extdata_wait =
                                rocprofvis_db_future_wait(object2wait4extdata,
                                                          UINT64_MAX);
                            REQUIRE(kRocProfVisDmResultSuccess == extdata_wait);
                            if(kRocProfVisDmResultSuccess == result4extdata &&
                               kRocProfVisDmResultSuccess == extdata_wait)
                            {
                                rocprofvis_dm_extdata_t extdata =
                                    rocprofvis_dm_get_property_as_handle(
                                        m_trace, kRPVDMExtInfoHandleByEventID,
                                        dm_event_id.value);
                                REQUIRE(extdata != nullptr);
                                spdlog::info("Extended data for event id = {}",
                                             (uint64_t) dm_event_id.bitfield.event_id);
                                uint64_t extdata_num_records =
                                    rocprofvis_dm_get_property_as_uint64(
                                        extdata, kRPVDMNumberOfExtDataRecordsUInt64, 0);
                                spdlog::info(ANSI_COLOR_MAGENTA
                                             "Event has {} extended data "
                                             "properties:",
                                             extdata_num_records);
                                for(int k = 0; k < extdata_num_records; k++)
                                {
                                    char* data_category =
                                        rocprofvis_dm_get_property_as_charptr(
                                            extdata, kRPVDMExtDataCategoryCharPtrIndexed,
                                            k);
                                    char* data_name =
                                        rocprofvis_dm_get_property_as_charptr(
                                            extdata, kRPVDMExtDataNameCharPtrIndexed, k);
                                    char* data_value =
                                        rocprofvis_dm_get_property_as_charptr(
                                            extdata, kRPVDMExtDataValueCharPtrIndexed, k);
                                    REQUIRE(data_category);
                                    REQUIRE(data_name);
                                    REQUIRE(data_value);
                                    uint64_t data_type =
                                        rocprofvis_dm_get_property_as_uint64(
                                            extdata, kRPVDMExtDataTypeUint64Indexed, k);
                                    uint64_t data_enum =
                                        rocprofvis_dm_get_property_as_uint64(
                                            extdata, kRPVDMExtDataEnumUint64Indexed, k);
                                    spdlog::info(ANSI_COLOR_MAGENTA
                                                 "\tItem {0} : category={1}, "
                                                 "name={2}, value={3}, "
                                                 "type={4}, enum={5} \n",
                                                 k, data_category, data_name, data_value,
                                                 data_type, data_enum);
                                }

                                uint64_t num_args = rocprofvis_dm_get_property_as_uint64(
                                    extdata, kRPVDMNumberOfArgumentRecordsUInt64, 0);
                                spdlog::info(ANSI_COLOR_MAGENTA
                                             "Event has {} argument records:",
                                             num_args);
                                for(uint64_t a = 0; a < num_args; a++)
                                {
                                    uint64_t arg_pos =
                                        rocprofvis_dm_get_property_as_uint64(
                                            extdata, kRPVDMArgumentPositionUint64Indexed,
                                            a);
                                    char* arg_type =
                                        rocprofvis_dm_get_property_as_charptr(
                                            extdata, kRPVDMArgumentTypeCharPtrIndexed, a);
                                    char* arg_name =
                                        rocprofvis_dm_get_property_as_charptr(
                                            extdata, kRPVDMArgumentNameCharPtrIndexed, a);
                                    char* arg_value =
                                        rocprofvis_dm_get_property_as_charptr(
                                            extdata, kRPVDMArgumentValueCharPtrIndexed,
                                            a);
                                    spdlog::info(ANSI_COLOR_MAGENTA
                                                 "\tArg {0} : pos={1}, type={2}, "
                                                 "name={3}, value={4}",
                                                 a, arg_pos,
                                                 arg_type ? arg_type : "(null)",
                                                 arg_name ? arg_name : "(null)",
                                                 arg_value ? arg_value : "(null)");
                                }

                                rocprofvis_dm_delete_event_property_for(
                                    m_trace, kRPVDMEventExtData, dm_event_id);
                            }
                            rocprofvis_db_future_free(object2wait4extdata);
                        }
                        else if(track_category ==
                                rocprofvis_dm_track_category_t::kRocProfVisDmPmcTrack)
                        {
                            double value = rocprofvis_dm_get_property_as_double(
                                slice, kRPVDMPmcValueDoubleIndexed, j);
                            spdlog::info(ANSI_COLOR_BLUE
                                         "Record timestamp={0}, value={1}\n",
                                         timestamp, value);
                        }
                    }
                }
                else
                {
                    spdlog::info(ANSI_COLOR_RED
                                 "No time slice at {0} loaded for track {1}",
                                 m_start_time, m_tracks_selection[i]);
                }
            }
            spdlog::info("Delete all slices");
            rocprofvis_dm_delete_all_time_slices(m_trace);

            rocprofvis_db_future_free(object2wait);
        }
    }

    // Executes a SQL query against the database, validates the resulting table
    // has columns and rows, and prints the content.
    // Fixture Reads: m_trace, m_db
    SECTION("SQL Query")
    {
        PrintHeader("Test SQL Query");
        CheckMemoryFootprint(m_trace);

        rocprofvis_db_future_t   object2wait = rocprofvis_db_future_alloc(progress_callback);
        rocprofvis_dm_table_id_t table_id    = 0;
        REQUIRE(nullptr != object2wait);
        spdlog::info("Test SQL table read");
        rocprofvis_dm_result_t query_result = rocprofvis_db_execute_query_async(
            m_db, "select * from top;", "Kernel execution summary", object2wait,
            &table_id);

        REQUIRE((kRocProfVisDmResultSuccess == query_result ||
                 kRocProfVisDmResultNotSupported == query_result));
        rocprofvis_dm_result_t query_wait =
            rocprofvis_db_future_wait(object2wait, UINT64_MAX);
        if(kRocProfVisDmResultSuccess == query_wait)
        {
            uint64_t num_tables = rocprofvis_dm_get_property_as_uint64(
                m_trace, kRPVDMNumberOfTablesUInt64, 0);
            if(num_tables > 0)
            {
                rocprofvis_dm_table_t table = rocprofvis_dm_get_property_as_handle(
                    m_trace, kRPVDMTableHandleByID, table_id);
                REQUIRE(table);
                char* table_description = rocprofvis_dm_get_property_as_charptr(
                    table, kRPVDMExtTableDescriptionCharPtr, 0);
                REQUIRE(table_description);
                char* table_query = rocprofvis_dm_get_property_as_charptr(
                    table, kRPVDMExtTableQueryCharPtr, 0);
                REQUIRE(table_query);
                uint64_t num_columns = rocprofvis_dm_get_property_as_uint64(
                    table, kRPVDMNumberOfTableColumnsUInt64, 0);
                uint64_t num_rows = rocprofvis_dm_get_property_as_uint64(
                    table, kRPVDMNumberOfTableRowsUInt64, 0);

                {
                    spdlog::info("\x1b[0m{0} - {1}", table_description, table_query);
                    MULTI_LINE_LOG_START;
                    std::vector<std::string> columns;
                    for(int i = 0; i < num_columns; i++)
                    {
                        columns.push_back(rocprofvis_dm_get_property_as_charptr(
                            table, kRPVDMExtTableColumnNameCharPtrIndexed, i));
                    }
                    for(int i = 0; i < num_columns; i++)
                    {
                        MULTI_LINE_LOG(" {0:20s} |", columns[i].c_str());
                    }
                    spdlog::info(MULTI_LINE_LOG_ARGS);
                }
                for(int i = 0; i < num_rows; i++)
                {
                    MULTI_LINE_LOG_START;
                    rocprofvis_dm_table_row_t table_row =
                        rocprofvis_dm_get_property_as_handle(
                            table, kRPVDMExtTableRowHandleIndexed, i);
                    REQUIRE(table_row);
                    if(table_row != nullptr)
                    {
                        uint64_t num_cells = rocprofvis_dm_get_property_as_uint64(
                            table_row, kRPVDMNumberOfTableRowCellsUInt64, 0);
                        if(num_cells == num_columns)
                        {
                            std::vector<std::string> row;
                            for(int j = 0; j < num_cells; j++)
                            {
                                row.push_back(rocprofvis_dm_get_property_as_charptr(
                                    table_row, kRPVDMExtTableRowCellValueCharPtrIndexed,
                                    j));
                            }
                            for(int j = 0; j < num_cells; j++)
                            {
                                MULTI_LINE_LOG("{0:20s}", row[j].substr(0, 20).c_str());
                            }
                        }
                        else
                        {
                            MULTI_LINE_LOG("\x1b[31mError! Number of colums does nt "
                                           "match number of cells in row {}\n",
                                           i);
                            break;
                        }
                    }
                    spdlog::info(MULTI_LINE_LOG_ARGS);
                }
                rocprofvis_dm_delete_all_tables(m_trace);
            }
        }
        rocprofvis_db_future_free(object2wait);
    }

    // Builds a structured table query via rocprofvis_db_build_table_query and validates
    // the resulting table has columns and rows.
    // Fixture Reads: m_trace, m_db, m_start_time, m_end_time
    SECTION("Structured Table Query")
    {
        CheckMemoryFootprint(m_trace);

        uint32_t    op[1] = { TABLE_QUERY_PACK_OP_TYPE(kRocProfVisDmOperationDispatch) };
        std::string sort_column             = "total_duration";
        char*       built_query             = nullptr;
        rocprofvis_dm_result_t build_result = rocprofvis_db_build_table_query(
            m_db, kRPVDMTableUseCaseEventTrackTable, m_start_time, m_end_time, 1,
            (rocprofvis_db_track_selection_t) op, nullptr, nullptr,
            "name, COUNT(*) AS num_invocations, AVG(duration) AS avg_duration, "
            "MIN(duration) AS min_duration, MAX(duration) AS max_duration, SUM(duration) "
            "AS total_duration",
            "name", sort_column.c_str(), kRPVDMSortOrderDesc, 0, 0, false, &built_query);
        REQUIRE(kRocProfVisDmResultSuccess == build_result);
        REQUIRE(built_query != nullptr);

        spdlog::info(ANSI_COLOR_GREEN "Built table query: {}", built_query);

        rocprofvis_db_future_t object2wait = rocprofvis_db_future_alloc(progress_callback);
        REQUIRE(nullptr != object2wait);

        rocprofvis_dm_table_id_t table_id    = 0;
        rocprofvis_dm_result_t   exec_result = rocprofvis_db_execute_query_async(
            m_db, built_query, "Structured table query test", object2wait, &table_id);
        REQUIRE(kRocProfVisDmResultSuccess == exec_result);

        rocprofvis_dm_result_t wait_result =
            rocprofvis_db_future_wait(object2wait, UINT64_MAX);
        REQUIRE(kRocProfVisDmResultSuccess == wait_result);
        uint64_t num_tables =
            rocprofvis_dm_get_property_as_uint64(m_trace, kRPVDMNumberOfTablesUInt64, 0);
        if(num_tables > 0)
        {
            rocprofvis_dm_table_t table = rocprofvis_dm_get_property_as_handle(
                m_trace, kRPVDMTableHandleByID, table_id);
            REQUIRE(table != nullptr);
            char* table_description = rocprofvis_dm_get_property_as_charptr(
                table, kRPVDMExtTableDescriptionCharPtr, 0);
            REQUIRE(table_description);
            uint64_t num_columns = rocprofvis_dm_get_property_as_uint64(
                table, kRPVDMNumberOfTableColumnsUInt64, 0);
            uint64_t num_rows = rocprofvis_dm_get_property_as_uint64(
                table, kRPVDMNumberOfTableRowsUInt64, 0);
            spdlog::info(ANSI_COLOR_GREEN
                         "Structured query result: desc={}, cols={}, rows={}",
                         table_description, num_columns, num_rows);
            REQUIRE(num_columns > 0);

            for(uint64_t r = 0; r < std::min(num_rows, (uint64_t) LIST_SIZE_LIMIT); r++)
            {
                rocprofvis_dm_table_row_t row = rocprofvis_dm_get_property_as_handle(
                    table, kRPVDMExtTableRowHandleIndexed, r);
                REQUIRE(row != nullptr);
                uint64_t num_cells = rocprofvis_dm_get_property_as_uint64(
                    row, kRPVDMNumberOfTableRowCellsUInt64, 0);
                REQUIRE(num_cells == num_columns);
            }

            rocprofvis_dm_delete_all_tables(m_trace);
        }
        free(built_query);
        rocprofvis_db_future_free(object2wait);
    }

    // Builds a structured table query, exports the result to a temporary CSV file
    // via rocprofvis_db_export_table_csv_async, validates the file exists and has
    // content, then removes it.
    // Fixture Reads: m_trace, m_db, m_start_time, m_end_time
    SECTION("Table CSV Export")
    {
        CheckMemoryFootprint(m_trace);

        uint32_t    op[1] = { TABLE_QUERY_PACK_OP_TYPE(kRocProfVisDmOperationDispatch) };
        std::string sort_column             = "total_duration";
        char*       csv_query               = nullptr;
        rocprofvis_dm_result_t build_result = rocprofvis_db_build_table_query(
            m_db, kRPVDMTableUseCaseEventTrackTable, m_start_time, m_end_time, 1,
            (rocprofvis_db_track_selection_t) op, nullptr, nullptr,
            "name, COUNT(*) AS num_invocations, AVG(duration) AS avg_duration, "
            "MIN(duration) AS min_duration, MAX(duration) AS max_duration, SUM(duration) "
            "AS total_duration",
            "name", sort_column.c_str(), kRPVDMSortOrderDesc, 0, 0, false, &csv_query);
        REQUIRE(kRocProfVisDmResultSuccess == build_result);
        REQUIRE(csv_query != nullptr);

        const char* csv_path = "sample/test_export.csv";

        rocprofvis_db_future_t object2wait = rocprofvis_db_future_alloc(progress_callback);
        REQUIRE(nullptr != object2wait);

        rocprofvis_dm_result_t export_result =
            rocprofvis_db_export_table_csv_async(m_db, csv_query, csv_path, object2wait);
        REQUIRE(kRocProfVisDmResultSuccess == export_result);
        rocprofvis_dm_result_t wait_result =
            rocprofvis_db_future_wait(object2wait, UINT64_MAX);
        REQUIRE(kRocProfVisDmResultSuccess == wait_result);
        REQUIRE(std::filesystem::exists(csv_path));
        uint64_t file_size = std::filesystem::file_size(csv_path);
        spdlog::info(ANSI_COLOR_GREEN "CSV export file size: {} bytes", file_size);
        REQUIRE(file_size > 0);

        std::filesystem::remove(csv_path);
        free(csv_query);
        rocprofvis_db_future_free(object2wait);
    }

    // Stress-tests concurrent read and delete operations on time slices across
    // multiple iterations to validate thread safety.
    // Fixture Reads:  m_trace, m_db, m_start_time, m_end_time, m_num_tracks
    // Fixture Writes: m_num_tracks, m_tracks_selection
    // Local Reads:    total_num_tracks
    SECTION("Thread Safety")
    {
        PrintHeader("Thread safety test");
        g_full_range = true;
        for(int test_count = 0; test_count < 10; test_count++)
        {
            rocprofvis_dm_timestamp_t start_time = m_start_time;
            rocprofvis_dm_timestamp_t end_time   = m_end_time;
            GenerateRandomSlice(m_trace, m_num_tracks, m_tracks_selection, start_time,
                                end_time);

            rocprofvis_db_future_t object2wait = rocprofvis_db_future_alloc(progress_callback);
            REQUIRE(nullptr != object2wait);

            rocprofvis_dm_result_t read_slice_issue =
                rocprofvis_db_read_trace_slice_async(m_db, start_time, end_time,
                                                     kRocProfVisDmHashedTimestampTagTrackSlice,
                                                     m_num_tracks, m_tracks_selection,
                                                     object2wait);
            REQUIRE(kRocProfVisDmResultSuccess == read_slice_issue);
            spdlog::info("Access data while loaded");
            {
                std::thread access_slice(ReadSliceData, m_trace, m_num_tracks, start_time,
                                         end_time, m_num_tracks * 20, object2wait);
                access_slice.join();
            }
            rocprofvis_dm_result_t read_wait =
                rocprofvis_db_future_wait(object2wait, UINT64_MAX);
            REQUIRE(kRocProfVisDmResultSuccess == read_wait);
            spdlog::info("Access data while deleted");
            std::thread delete_slice(DeleteSliceData, m_trace, start_time, end_time, 100);
            {
                std::thread access_slice(ReadSliceData, m_trace, m_num_tracks, start_time,
                                         end_time, 99, nullptr);
                delete_slice.join();
                access_slice.join();
            }
            rocprofvis_db_future_free(object2wait);
        }
    }

    // Cleans the database with rebuild enabled.
    // Fixture Reads: m_trace, m_db
    SECTION("Database Cleanup")
    {
        CheckMemoryFootprint(m_trace);

        uint64_t initial_size = std::filesystem::file_size(g_input_file);
        spdlog::info("Initial DB file size: {} bytes", initial_size);
        REQUIRE(initial_size > 0);

        rocprofvis_db_future_t object2wait = rocprofvis_db_future_alloc(progress_callback);
        REQUIRE(nullptr != object2wait);

        rocprofvis_dm_result_t result =
            rocprofvis_db_cleanup_async(m_db, object2wait, true);
        REQUIRE(kRocProfVisDmResultSuccess == result);

        result = rocprofvis_db_future_wait(object2wait, UINT64_MAX);
        REQUIRE(kRocProfVisDmResultSuccess == result);

        uint64_t final_size = std::filesystem::file_size(g_input_file);
        spdlog::info("Final DB file size: {} bytes", final_size);

        rocprofvis_db_future_free(object2wait);
    }

    // Reports memory footprint and frees the trace handle.
    // Fixture Reads: m_trace
    SECTION("Delete Trace")
    {
        CheckMemoryFootprint(m_trace);
        PrintHeader("Delete trace");
        rocprofvis_dm_delete_trace(m_trace);
    }
}
