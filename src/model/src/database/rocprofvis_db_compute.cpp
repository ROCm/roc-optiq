...
namespace DataModel
{
	...
	std::string ComputeQueryFactory::GetComputeListOfWorkloads(rocprofvis_db_num_of_params_t num, rocprofvis_db_compute_params_t params) {
		...
		// memory_chart_extdata is an optional extension column; only select it
		// when the database actually has it so older databases still load.
		if (m_db != nullptr && m_db->m_has_memory_chart_extdata)
		{
			query += ", memory_chart_extdata";
		}
		query += " FROM ";
