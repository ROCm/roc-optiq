...
rocprofvis_result_t ComputeDataProvider::LoadTrace(rocprofvis_controller_t* controller)
{
    m_tables.clear();
    m_plots.clear();
    m_metrics.clear();
    ...
    if (result == kRocProfVisResultSuccess)
    {
        ...
        if (result == kRocProfVisResultSuccess)
        {
            ...
            {
                result = rocprofvis_controller_get_object(m_controller, table_type, 0, &table_handle);
                if (result == kRocProfVisResultSuccess)
                {
                    ...
                    if (result == kRocProfVisResultSuccess)
                    {
                        ...
                        if (result == kRocProfVisResultSuccess)
                        {
                            ...
                            result = rocprofvis_controller_table_fetch_async(m_controller, table_handle, sort, table_future, table_data);
                            if (result == kRocProfVisResultSuccess)
                            {
            ...
            {
                result = rocprofvis_controller_get_object(m_controller, plot_type, 0, &plot_handle);
                if (result == kRocProfVisResultSuccess)
                {
                                            ...
                                            for (uint64_t j = 0; j < num_values; j ++)
                                            {
                                                ...
                                                if (result == kRocProfVisResultSuccess)
                                                {
                                                    ...
                                                    if (result == kRocProfVisResultSuccess)
                                                    {
                                                        ...
                                                        if (result == kRocProfVisResultSuccess && name.length() > PLOT_LABEL_MAX_LENGTH)
                                                        {
                                                            size_t dash = name.find_last_of('-');
                                                            if (dash != std::string::npos)
                                                            {                                                                    
                                                                name.replace(dash - (name.length() - PLOT_LABEL_MAX_LENGTH), (name.length() - PLOT_LABEL_MAX_LENGTH), "...");
                                                            }
                                                            else
                                                            {
                                                                name = name.substr(0, PLOT_LABEL_MAX_LENGTH) + "...";
                                                            }
                                                        }
                                                        x_axis.m_max = (i == 0 && j == 0) ? x_values[j] : std::max(x_axis.m_max, x_values[j]);
                                                        x_axis.m_min = (i == 0 && j == 0) ? x_values[j] : std::min(x_axis.m_min, x_values[j]);                                                            
                                                        y_axis.m_max = (i == 0 && j == 0) ? y_values[j] : std::max(y_axis.m_max, y_values[j]);
                                                        y_axis.m_min = (i == 0 && j == 0) ? y_values[j] : std::min(y_axis.m_min, y_values[j]);
                                                        if (x_values[j] > 0)
                                                        {
                                                            x_axis.m_min_non_zero = (i == 0 && j == 0) ? x_values[j] : std::min(x_axis.m_min_non_zero, x_values[j]);
                                                        }
                                                        if (y_values[j] > 0)
                                                        {
                                                            y_axis.m_min_non_zero = (i == 0 && j == 0) ? y_values[j] : std::min(y_axis.m_min_non_zero, y_values[j]);
                                                        }                                                            
                                                    }
                                                }
                                            }
            ...
            {
                uint64_t data;
                result = rocprofvis_controller_get_uint64(m_controller, metric_type, 0, &data);
                if (result == kRocProfVisResultSuccess)
                {
...
ComputeTableModel* ComputeDataProvider::GetTableModel(const rocprofvis_controller_compute_table_types_t type) const
{
...
ComputePlotModel* ComputeDataProvider::GetPlotModel(const rocprofvis_controller_compute_plot_types_t type) const
{
...
ComputeMetricModel* ComputeDataProvider::GetMetricModel(const rocprofvis_controller_compute_metric_types_t type) const
{
...
rocprofvis_result_t ComputeDataProvider::GetStringPropertyFromHandle(rocprofvis_handle_t* handle, const rocprofvis_property_t property, const uint64_t index, std::string& output)
{
    uint32_t length = -1;
...
std::string ComputeDataProvider::TrimDecimalPlaces(std::string& double_str, const int decimal_places)
{
