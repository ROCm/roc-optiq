                                    ...
                                    char** azColName)
{
    ...
    if(callback_params->future->GetProcessedRowsCount() == 0)
    {
        for(int i = 0; i < argc; i++)
        {
            ref_tables->AddTableColumn(
                callback_params->query[kRPVCacheTableName].c_str(), azColName[i],
    ...
    for(int i = 0; i < argc; i++)
    {
        ...
        {
            ref_tables->AddTableCell(callback_params->query[kRPVCacheTableName].c_str(),
                                     id, i, callback_params->query[kRPVCacheTableName]);
        }
        else
        {
            ref_tables->AddTableCell(
                callback_params->query[kRPVCacheTableName].c_str(), id, i,
                                   ...
                                   char** azColName)
{
    ...
    // track names are empty here -> so probably in processTrack we will get their names

    spdlog::info("Track row (cached): category={} op={} process_id={} record_count={} "
                 "id0={} id1={} id2={}",
                 (int) track_params.track_indentifiers.category, track_params.op,
                 track_params.track_indentifiers.process_id, track_params.record_count,
                 track_params.track_indentifiers.id[0],
                 track_params.track_indentifiers.id[1],
                 track_params.track_indentifiers.id[2]);
...
bool
ProfileDatabase::FindTrack(rocprofvis_dm_track_category_t category, uint64_t id_process,
                           uint64_t id_subprocess, uint32_t db_instance,
                           uint32_t& out_track)
{
    return TrackTracker()->FindTrack(category, id_process, id_subprocess, db_instance,
                                     out_track);
}
    ...
    for(auto& file_node : m_db_nodes)
    {
        ...
        else
        {
            ...
            result = ExecuteQueryForAllTracksAsync(
                ...
                kRPVRocpdQuerySliceByTrackSliceQuery, histogram_query_prefix.c_str(),
            ...
            if(kRocProfVisDmResultSuccess == result)
            {
                ...
                result = ExecuteQueryForAllTracksAsync(
                    ...
                    kRPVRocpdQuerySliceByTrackSliceQuery, histogram_query.c_str(),
                                     ...
                                     std::vector<rocprofvis_dm_string_t>& newqueries)
{
    int slice_query_category =
        newprops.track_indentifiers.category == kRocProfVisDmStreamTrack
            ? kRPVRocpdQuerySliceByStream
            : kRPVRocpdQuerySliceByQueue;
    int slice_source_query_category =
        newprops.track_indentifiers.category == kRocProfVisDmStreamTrack
            ? kRPVSourceQuerySliceByStream
            : kRPVSourceQuerySliceByQueue;
    if(it != TrackPropertiesEnd())
    {
        std::vector<rocprofvis_dm_string_t>::iterator s = std::find_if(
            it->get()->query[slice_query_category].begin(),
        ...
        {
            it->get()->query[kRPVRocpdQueryTable].push_back(
                newqueries[kRPVSourceQueryTable]);
        }
        ...
        {
            it->get()->query[kRPVRocpdQueryLevel].push_back(
                newqueries[kRPVSourceQueryLevel]);
        }
        return;
    }
    newprops.query[slice_query_category].push_back(
        newqueries[slice_source_query_category]);
