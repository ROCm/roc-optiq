                                        ...
                                        for (uint32_t idx = 0; idx < num_ext_data; idx++)
                                        {
                                            ...
                                            if (category == "Queue" && name == "id")
                                            {
                                                ...
                                                uint64_t val = std::strtoull(
                                                    value.c_str(), &end, 10);
                                                queue_to_track.insert({ val, track });
                                            }
                                            else if(category == "Stream" && name == "id")
                                            {
                                                ...
                                                uint64_t val = std::strtoull(
                                                    value.c_str(), &end, 10);
                                                stream_to_track.insert({ val, track });
                                            }
                                                    ...
                                                    if(num_cells == num_columns)
                                                    {
                                                        ...
                                                        for(int j = 0; j < num_cells; j++)
                                                        {
                                                            ...
                                                            else if(columns[j] == kRPVControllerStreamId)
                                                            {
                                                                ...
                                                                auto it = processors.find(stream_to_device[stream_id]);
                                                        ...
                                                        for(auto stream : streams)
                                                        {
                                                            ...
                                                            auto& set = stream_to_queue[stream_id];
        ...
        [this, property, &array, index](Future* future) -> rocprofvis_result_t {
            ...
            switch(property)
            {
                ...
                case kRPVControllerBucketDataValueIndexed:
                {
                    ...
                    const size_t buckets_num_size = static_cast<size_t>(buckets_num);
                    result = m_tracks[index]->GetBucketValues(buckets_num_size, array);
                    break;
                }
                default:
                {
