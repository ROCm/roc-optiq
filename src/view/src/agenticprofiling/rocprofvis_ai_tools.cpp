...
// Maps the name the model used to the body that implements it. Kept beside
// the bodies so adding a tool is one entry and one function, and so a name
// that is registered in the schema but missing here fails loudly.
...
const AssistantToolEntry ASSISTANT_TOOL_HANDLERS[] = {
                   ...
                   const std::string& arguments_json)
{
    ...
    for(const AssistantToolEntry& entry : ASSISTANT_TOOL_HANDLERS)
    {
        if(tool_name == entry.name)
        {
            return entry.handler(context, args, tool_name);
        }
    }
