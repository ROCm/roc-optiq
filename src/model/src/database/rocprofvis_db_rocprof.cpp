                              ...
                              std::vector<rocprofvis_dm_string_t>& newqueries)
{
    ...
    if(it == TrackPropertiesEnd())
    {
           ...
           track_params.track_indentifiers.category == kRocProfVisDmRegionSampleTrack)
        {
            ...
            track_params.track_indentifiers.name[TRACK_ID_TID] += ")";
            spdlog::info(
                "ProcessTrack category == RegionMainTrack or RegionSampleTrack. Name{} ",
                track_params.track_indentifiers.name[TRACK_ID_TID]);
        }
...
RocprofDatabase::LoadInformationTables(Future* future)
{
    ...
    std::vector<std::pair<std::string, std::string>> info_table_list = {
        ...
        { "Agent",
          "SELECT "
          "id,guid,nid,pid,type,absolute_index,logical_index,type_index,uuid,name,model_"
          "name,vendor_name,product_name,extdata from rocpd_info_agent_%GUID%;" },
    ...
    while(true)
    {
        ...
        for(auto& guid_info : DbInstances())
        {
            ...
            if(!version.empty())
            {
                ...
                {
                    spdlog::warn("Schema mismatch: all database sources must use the "
                                 "same schema version; the trace cannot be opened.");
        ...
        ShowProgress(5, "Adding HIP API tracks", kRPVDbBusy, future);
        {
            ...
            auto task = [&](DbInstance* db_instance) {
                Future* sub_future = future->AddSubFuture();
                result =
                    ExecuteSQLQuery(sub_future, db_instance, load_id,
                                    {
        ...
        if(calculate_level_for_guids.size())
        {
            if(kRocProfVisDmResultSuccess !=
               ExecuteQueryForAllTracksAsync(
                   kRocProfVisDmIncludeStreamTracks, kRPVRocpdQueryLevel, "SELECT *, ",
...
RocprofDatabase::ReadFlowTraceInfo(rocprofvis_dm_event_id_t event_id, Future* future)
{
    ...
    while(true)
    {
        ...
        {
            query = m_query_factory.GetRocprofDataFlowQueryForRegionEvent(
            ...
            if(kRocProfVisDmResultSuccess != ExecuteSQLQuery(future, node_ptr,
                                                             query.c_str(), flowtrace,
                                                             &CallbackAddFlowTrace))
                break;
        }
        else if(event_id.bitfield.event_op == kRocProfVisDmOperationDispatch)
        {
            query = m_query_factory.GetRocprofDataFlowQueryForKernelDispatchEvent(
                event_id.bitfield.event_id);
            ShowProgress(0, query.c_str(), kRPVDbBusy, future);
            if(kRocProfVisDmResultSuccess != ExecuteSQLQuery(future, node_ptr,
                                                             query.c_str(), flowtrace,
        ...
        {
            query = m_query_factory.GetRocprofDataFlowQueryForMemoryCopyEvent(
                event_id.bitfield.event_id);
            ShowProgress(0, query.c_str(), kRPVDbBusy, future);
            if(kRocProfVisDmResultSuccess != ExecuteSQLQuery(future, node_ptr,
                                                             query.c_str(), flowtrace,
        ...
        {
            query = m_query_factory.GetRocprofDataFlowQueryForMemoryAllocEvent(
                event_id.bitfield.event_id);
            ShowProgress(0, query.c_str(), kRPVDbBusy, future);
            if(kRocProfVisDmResultSuccess != ExecuteSQLQuery(future, node_ptr,
                                                             query.c_str(), flowtrace,
...
RocprofDatabase::BuildTableStringIdFilter(
    ...
    bool include_category, bool partial_matching, table_string_id_filter_map_t& filter)
{
    rocprofvis_dm_result_t result = kRocProfVisDmResultNotLoaded;
    if(num_string_table_filters > 0)
    {
        std::vector<rocprofvis_dm_index_t> string_indices;
        result = BindObject()->FuncGetStringIndices(
            BindObject()->trace_object, num_string_table_filters, string_table_filters,
            include_substring, partial_matching, string_indices);
        ...
        for(const rocprofvis_dm_index_t& index : string_indices)
        {
            ...
            for(const auto& string_id_obj : string_id_array)
            {
                   ...
                   rocprofvis_db_string_type_t::kRPVStringTypeKernelSymbol)
                {
                    ...
                    {
                        kernel_symbol[string_id_obj.m_guid_id] =
                ...
                else
                {
                    ...
                    {
                        name_or_category[string_id_obj.m_guid_id] =
                            std::to_string(string_id_obj.m_string_id);
                    }
                    else
                    {
        ...
        for(const std::string& type : Builder::mem_alloc_types)
        {
            ...
            for(rocprofvis_dm_num_string_table_filters_t i = 0;
                i < num_string_table_filters; i++)
            {
                std::string_view target_sv = string_table_filters[i];
                if(include_substring)
                {
                    match = std::search(type.begin(), type.end(), target_sv.begin(),
                                        target_sv.end(),
            ...
            {
                mem_alloc_types +=
                    mem_alloc_types.empty() ? '\'' + type + '\'' : ", '" + type + '\'';
            }
        }
        ...
        if(mem_alloc_types.size())
        {
            ...
            {
                filter[kRocProfVisDmOperationMemoryAllocate][instance.first.GuidIndex()] =
                    std::string(Builder::M_TYPE_REFERENCE) + " IN (" + mem_alloc_types;
            }
        }
        if(name_or_category.size() > 0)
        {
            for(auto it = name_or_category.begin(); it != name_or_category.end(); ++it)
            {
                ...
                {
                    filter[kRocProfVisDmOperationLaunch][it->first] +=
                        ") OR E.category_id IN (" + it->second;
                    filter[kRocProfVisDmOperationDispatch][it->first] +=
                        ") OR E.category_id IN (" + it->second;
                    filter[kRocProfVisDmOperationMemoryAllocate][it->first] =
                        filter[kRocProfVisDmOperationMemoryAllocate][it->first].empty()
                            ? " E.category_id IN (" + it->second
                            : "(" +
                                  filter[kRocProfVisDmOperationMemoryAllocate]
                                        [it->first] +
                                  ") OR E.category_id IN (" + it->second + ")";
                    filter[kRocProfVisDmOperationMemoryCopy][it->first] +=
                        ") OR E.category_id IN (" + it->second;
