                            ...
                            rocprofvis_dm_timestamp_t & end_time)
{
    ...
    PrintHeader("Testing slice for %lld ns and %ld tracks", end_time - start_time, count);
...
int main(int argc, char** argv)
{
    ...
    if (argc < 1) {
        printf("USAGE: rocprofvis-datamodel-lib-test [database name]\n");
    }
    ...
    if (nullptr != trace) 
    {
        ...
        PrintHeader("Open database %s", argv[1]);
        rocprofvis_dm_database_t db = rocprofvis_db_open_database(argv[1], kAutodetect);
                ...
                if (kRocProfVisDmResultSuccess == rocprofvis_db_read_metadata_async(db, object2wait))
                {
	                ...
	                {   
                        ...
                        for (int i = 0; i < num_tracks; i++)
                        {
                            ...
                            printf(ANSI_COLOR_CYAN "Track id=%lld node=%lld category=%s process=%s subprocess=%s\n",
                            ...
                            if (kRocProfVisDmResultSuccess == rocprofvis_db_read_trace_slice_async(db, start_time, end_time, num_tracks, tracks_selection, object2wait))
                            {
                                if (kRocProfVisDmResultSuccess == rocprofvis_db_future_wait(object2wait, 10))
                                {
                                    ...
                                    for (int i=first_track; (i < num_tracks) && (i < first_track+ LIST_SIZE_LIMIT); i++)
                                    {
                                        ...
                                        roprofvis_dm_track_category_t track_category = (roprofvis_dm_track_category_t)rocprofvis_dm_get_property_as_uint64(track, kRPVDMTrackCategoryEnumUInt64, 0);
                                        ...
                                        if (nullptr != slice)
                                        {
                                            ...
                                            for (int j = first_record; (j < num_records) && (j < first_record+ LIST_SIZE_LIMIT); j++)
                                            {
                                                ...
                                                if (track_category == roprofvis_dm_track_category_t::kRocProfVisDmRegionTrack || track_category == roprofvis_dm_track_category_t::kRocProfVisDmKernelTrack)
                                                {
                                                    int64_t duration = rocprofvis_dm_get_property_as_int64(slice, kRPVDMEventDurationInt64Indexed, j);
                                                    if (duration < 0)
                                                    {
                                                        printf(ANSI_COLOR_RED "Record %ld has invalid duration %lld\n" ANSI_COLOR_RESET, j, duration);
                                                    }
                                                    ...
                                                    PrintHeader("Testing multithreaded access to database retrieving flow, stack and extended data for event id = %lld", event_id.bitfiled.event_id);
                                                    ...
                                                    if (result4flowtrace && kRocProfVisDmResultSuccess == rocprofvis_db_future_wait(object2wait4flowtrace, 1))
                                                    {
                                                        ...
                                                        if (flowtrace != nullptr) {
                                                            PrintHeader("Data flow trace for event id = %lld", event_id.bitfiled.event_id);
                                                    ...
                                                    if (result4stacktrace && kRocProfVisDmResultSuccess == rocprofvis_db_future_wait(object2wait4stacktrace, 1))
                                                    {
                                                        ...
                                                        if (stacktrace != nullptr) {
                                                            PrintHeader("Stack trace for event id = %lld", event_id.bitfiled.event_id);
                                                    ...
                                                    if (result4extdata && kRocProfVisDmResultSuccess == rocprofvis_db_future_wait(object2wait4extdata, 1))
                                                    {
                                                        rocprofvis_dm_extdata_t extdata = rocprofvis_dm_get_property_as_handle(trace, kRPVDMExtInfoHandleByEventID, event_id.value);
                                                        if (extdata != nullptr) {
                                                            PrintHeader("Extended data for event id = %lld", event_id.bitfiled.event_id);
                                                    ...
                                                    if (track_category == roprofvis_dm_track_category_t::kRocProfVisDmPmcTrack)
                                                    {
                                            ...
                                            printf(ANSI_COLOR_RED "No time slice at %lld loaded for track %ld\n",start_time,tracks_selection[i]);
                                        }
                                    }
