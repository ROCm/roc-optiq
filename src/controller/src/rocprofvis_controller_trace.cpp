                    ...
                    for(auto& thread : data.m_threads)
                    {
                        ...
                        if(track)
                        {
                            track->SetString(kRPVControllerTrackName, 0,
                                             thread.first.c_str(), thread.first.length());
                            switch(type)
                            {
                                case kRPVControllerTrackTypeEvents:
                                {
                                    track->SetUInt64(kRPVControllerTrackNumberOfEntries,
                                                     0, thread.second.m_events.size());
                                    ...
                                    for(auto& event : thread.second.m_events)
                                    {
                                        ...
                                        max_ts = std::max(
                                            event.m_start_ts + event.m_duration, max_ts);
                                    }
                                    track->SetDouble(kRPVControllerTrackMinTimestamp, 0,
                                                     min_ts);
                                    track->SetDouble(kRPVControllerTrackMaxTimestamp, 0,
                                                     max_ts);
                                    ...
                                    if(result == kRocProfVisResultSuccess)
                                    {
                                        ...
                                        {
                                            graph = new Graph(
                                                kRPVControllerGraphTypeFlame, graph_id++);
                                            if(graph)
                                            {
                                ...
                                case kRPVControllerTrackTypeSamples:
                                {
                                    ...
                                    if(result == kRocProfVisResultSuccess)
                                    {
                                        ...
                                        if(m_tracks.size() == (index + 1))
                                        {
                                            graph = new Graph(kRPVControllerGraphTypeLine,
                                                              graph_id++);
                                            if(graph)
                                            {
                                                result = graph->SetObject(
