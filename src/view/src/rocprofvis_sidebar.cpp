        ...
        for(int i = 0; i < node_infos.size(); i++)
        {
            ...
            if(node_info)
            {
                ...
                for(const uint64_t& device_id : node_info->device_ids)
                {
                    ...
                    if(device_info)
                    {
                        m_project.nodes[i].info_table.cells.emplace_back(std::vector{
                ...
                m_project.nodes[i].process_header =
                    "Processes (" + std::to_string(process_ids.size()) + ")";
                for(int j = 0; j < process_ids.size(); j++)
                {
                    m_project.nodes[i].process_lut[process_ids[j]] =
                        &m_project.nodes[i].processes[j];
                    ...
                    if(process_info)
                    {
                        ...
                        m_project.nodes[i].processes[j].header =
                        ...
                        m_project.nodes[i].processes[j].queue_header =
                            "Queues (" + std::to_string(queue_ids.size()) + ")";
                        for(int k = 0; k < queue_ids.size(); k++)
                        {
                            m_project.nodes[i].processes[j].queue_lut[queue_ids[k]] =
                                &m_project.nodes[i].processes[j].queues[k];
                            ...
                            if(queue_info)
                            {
                                ...
                                m_project.nodes[i].processes[j].queues[k].info =
                                    queue_info;
                                if(device_info)
                                {
                                    m_project.nodes[i].processes[j].queues[k].info_table =
                                        InfoTable{
                                            { { InfoTable::Cell{
                                                    device_info->type + " " +
                                                        std::to_string(
                                                    ...
                                                    false },
                                                InfoTable::Cell{
                                                    device_info->product_name, false } } }
                                        };
                                }
                            }
                        }
                        ...
                        m_project.nodes[i].processes[j].thread_header =
                            "Threads (" + std::to_string(thread_ids.size()) + ")";
                        for(int k = 0; k < thread_ids.size(); k++)
                        {
                            m_project.nodes[i].processes[j].thread_lut[thread_ids[k]] =
                                &m_project.nodes[i].processes[j].threads[k];
                            const thread_info_t* thread_info =
                                m_data_provider.GetThreadInfo(thread_ids[k]);
                            if(thread_info)
                            {
                                m_project.nodes[i].processes[j].threads[k].info =
                                    thread_info;
                                m_project.nodes[i].processes[j].threads[k].info_table =
                                    InfoTable{ {
                                        { InfoTable::Cell{ "Start Time", false },
                                          InfoTable::Cell{
                        ...
                        m_project.nodes[i].processes[j].counters.resize(
                            counter_ids.size());
                        m_project.nodes[i].processes[j].counter_header =
                            "Counters (" + std::to_string(counter_ids.size()) + ")";
                        for(int k = 0; k < counter_ids.size(); k++)
                        {
                            m_project.nodes[i].processes[j].counter_lut[counter_ids[k]] =
                                &m_project.nodes[i].processes[j].counters[k];
                            ...
                            if(counter_info)
                            {
                                ...
                                m_project.nodes[i].processes[j].counters[k].info =
                                    counter_info;
                                m_project.nodes[i].processes[j].counters[k].info_table =
                                    InfoTable{
...
SideBar::Render()
{
    if(!m_topology_dirty && !m_graphs_dirty)
    {
        ...
        ImGui::SetNextWindowContentSize(ImVec2(
            ImGui::GetContentRegionAvail().x - ImGui::GetStyle().ScrollbarSize, 0));
        ...
        if(ImGui::CollapsingHeader(m_project.header.c_str(), CATEGORY_HEADER_FLAGS))
        {
            ...
            if(!m_project.nodes.empty() &&
               ImGui::CollapsingHeader(m_project.node_header.c_str(),
                    ...
                    for(const int& index : m_project.uncategorized_graph_indices)
                    {
                        ...
                        if(ImGui::CollapsingHeader(track_name.c_str()))
                        {
                            ...
                            ImGui::GetWindowDrawList()->AddRect(
                                ...
                                ImGui::GetColorU32(
                                    ImGui::GetStyleColorVec4(ImGuiCol_TableBorderLight)));
                            ImGui::Unindent();
                        }
                        ImGui::PopID();
                    }
...
SideBar::RenderTrackItem(const int& index)
{
    ...
    std::string label =
        graph.chart->GetName() + " ID:" + std::to_string(graph.chart->GetID());
    ...
    {
        if(ImGui::Checkbox("Enable/Disable Color", &graph.colorful_flamechart))
    ...
    if(graph.graph_type == rocprofvis_graph_t::TYPE_LINECHART)
    {
        ...
        ImGui::Checkbox("Convert to Boxplot", &graph.make_boxplot);
        ...
        if(graph.color_by_value)
        {
            ...
            ImGui::InputFloat("Region of Interest Min",
                              &graph.color_by_value_digits.interest_1_min);

            ImGui::InputFloat("Region of Interest Max",
