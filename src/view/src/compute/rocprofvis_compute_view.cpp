...
ComputeTester::RenderFetcher(const WorkloadInfo& workload)
{
    ...
    if(ImGui::Button("Fetch", ImVec2(-1.0f, 0.0f)))
    {
        ...
        {
            for(const std::pair<const uint32_t,
                                std::pair<bool, std::unordered_set<uint32_t>>>& table :
                category.second)
            {
                if(table.second.second.empty())
                {
                                  ...
                                  (1.0f + static_cast<float>(metrics.size())))))
    {
        ...
        for(const std::unique_ptr<MetricValue>& metric : metrics)
        {
            if(metric)
            {
                ImGui::TableNextRow();
                ImGui::TableNextColumn();
                ImGui::Text("%u.%u.%u", metric->entry.category_id, metric->entry.table_id,
                            metric->entry.id);
                ImGui::TableNextColumn();
...
void
ComputeTester::RenderRoofLine(const WorkloadInfo& workload)
{
