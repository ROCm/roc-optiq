...
**Kernel Details** focuses on one kernel at a time. It provides:  
...
The search box below each column's header allows you to enter statements to filter the data, ensuring targeted analysis. 
Click **Apply Filters** to execute. You can search for a kernel by name or metric equation (for example, metricA > threshold).  
...
Selecting a kernel either through the **Kernel Selection Table** or kernel selector drop-down updates Memory Chart, System Speed-of-Light, Kernel-level Roofline Analysis and Table View accordingly. 
...
Displays memory transactions and throughput on each cache hierarchical level. 
This visual diagram shows counter values and calculations in a way users can understand which cache level it corresponds to and how they interact with each other.  
...
Displays key kernel-level performance metrics to show overall compute performance and hardware utilization.  

Kernel Roofline Chart
---------------------

Displays a kernel-specific roofline analysis which helps you determine whether the kernel is compute-bound or memory-bound. 
...
Metrics are grouped into tabs that match compute categories, such as: 
