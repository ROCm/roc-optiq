...
void RegisterAppTests(ImGuiTestEngine* e)
{
    ...
    // AIPROFVIS-333: dragging the Description-column splitter must resize the
    // sidebar only; the timeline pan (view min-x) must stay fixed. RenderSplitter
    // currently compensates the drag into SetViewTimeOffsetNs, so the view pans
    // with the resize -- this test fails until that compensation is removed.
    ...
    t->TestFunc = [](ImGuiTestContext* ctx)
    {
        ...
        // Restore runs before the asserts, not after: the harness shares one
        // process and IM_CHECK returns on failure, so a trailing restore would
        // leak zoom/offset into later tests while the bug is unfixed.
