...
namespace DataModel
{
    typedef struct rocprofvis_db_track_identifier_index_t
    {
    ...
    public:
        ...
        Numeric GetMergeTableValue(uint8_t op, size_t row, size_t col) const;
        ...
        DbInstance* GetDbInstanceForRow(SystemDatabase * db, int row_index);
        DbInstance* GetDbInstanceForRow(SystemDatabase* db, PackedRow* row);
        ...
        void SortByColumn(SystemDatabase * db, std::string column, bool ascending);
        bool SetupAggregation(std::string agg_spec, int num_threads);
        void FinalizeAggregation();
        void ClearAggregation();
        void AggregateRow(SystemDatabase * db, int row_index, int map_index);
        void SortAggregationByColumn(std::string sort_column, bool sort_order);
        void RemoveRowsForSetOfTracks(std::set<uint32_t> & selected_tracks, std::set<uint32_t> & unselected_tracks, bool remove_all);

        static const char* ConvertSqlStringReference(SystemDatabase* db, uint32_t column_index, uint64_t index, uint32_t node_id, bool & numeric_string);
