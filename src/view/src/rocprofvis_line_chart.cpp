    ...
    if(ImGui::BeginChild((std::to_string(m_id)).c_str()), true, window_flags)
    {
        ...
        for(int i = 1; i < m_data.size(); i++)
        {
            ...
            if(is_color_value_existant)
            {
                ...
                bool point_2_in = (m_color_by_value_digits.upper_max > m_data[i].yValue &&
                ...
                {
                    if(m_color_by_value_digits.upper_max < m_data[i - 1].yValue){
                    
                          float new_y =
                            cursor_position.y + content_size.y -
                            (m_color_by_value_digits.upper_max - m_min_y) * scale_y;
                    ...
                    else if(m_color_by_value_digits.upper_min > m_data[i - 1].yValue)
                    {
                        float new_y =
                            cursor_position.y + content_size.y -
                            (m_color_by_value_digits.upper_min - m_min_y) * scale_y;
                        float new_x = CalculateMissingX(point_1.x, point_1.y, point_2.x,
                                                        point_2.y, new_y);
                ...
                else if(point_1_in && !point_2_in)
                {
                    ...
                    else if(m_color_by_value_digits.upper_min > m_data[i].yValue)
                    {
                        float new_y =
                            cursor_position.y + content_size.y -
                            (m_color_by_value_digits.upper_min - m_min_y) * scale_y;
