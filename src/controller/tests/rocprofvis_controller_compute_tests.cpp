...
TEST_CASE("PC sampling stalls can omit instruction sample metadata", "[pc_sampling]")
{
    using ControllerPtr =
        std::unique_ptr<rocprofvis_controller_t, decltype(&rocprofvis_controller_free)>;
    using ArgumentsPtr = std::unique_ptr<rocprofvis_controller_arguments_t,
                                         decltype(&rocprofvis_controller_arguments_free)>;
    using FuturePtr    = std::unique_ptr<rocprofvis_controller_future_t,
                                         decltype(&rocprofvis_controller_future_free)>;

    ControllerPtr controller(rocprofvis_controller_alloc(g_input_file.c_str(), nullptr),
                             &rocprofvis_controller_free);
    REQUIRE(controller != nullptr);

    FuturePtr load_future(rocprofvis_controller_future_alloc(),
                          &rocprofvis_controller_future_free);
    REQUIRE(load_future != nullptr);
    REQUIRE(rocprofvis_controller_load_async(controller.get(), load_future.get()) ==
            kRocProfVisResultSuccess);
    REQUIRE(rocprofvis_controller_future_wait(load_future.get(), FLT_MAX) ==
            kRocProfVisResultSuccess);
    uint64_t load_result = std::numeric_limits<uint64_t>::max();
    REQUIRE(rocprofvis_controller_get_uint64(load_future.get(),
                                             kRPVControllerFutureResult, 0,
                                             &load_result) == kRocProfVisResultSuccess);
    REQUIRE(load_result == kRocProfVisResultSuccess);

    uint64_t workload_count = 0;
    REQUIRE(rocprofvis_controller_get_uint64(controller.get(), kRPVControllerNumWorkloads,
                                             0, &workload_count) ==
            kRocProfVisResultSuccess);

    uint64_t             kernel_id   = 0;
    rocprofvis_handle_t* pc_sampling = nullptr;
    for(uint64_t workload_index = 0; workload_index < workload_count && !pc_sampling;
        ++workload_index)
    {
        rocprofvis_handle_t* workload = nullptr;
        REQUIRE(rocprofvis_controller_get_object(
                    controller.get(), kRPVControllerWorkloadIndexed, workload_index,
                    &workload) == kRocProfVisResultSuccess);

        uint64_t kernel_count = 0;
        REQUIRE(rocprofvis_controller_get_uint64(
                    workload, kRPVControllerWorkloadNumKernels, 0, &kernel_count) ==
                kRocProfVisResultSuccess);
        for(uint64_t kernel_index = 0; kernel_index < kernel_count && !pc_sampling;
            ++kernel_index)
        {
            rocprofvis_handle_t* kernel = nullptr;
            REQUIRE(rocprofvis_controller_get_object(
                        workload, kRPVControllerWorkloadKernelIndexed, kernel_index,
                        &kernel) == kRocProfVisResultSuccess);

            uint64_t has_isa_lines = 0;
            REQUIRE(rocprofvis_controller_get_uint64(
                        kernel, kRPVControllerKernelHasIsaLines, 0, &has_isa_lines) ==
                    kRocProfVisResultSuccess);
            if(!has_isa_lines)
            {
                continue;
            ...
            REQUIRE(rocprofvis_controller_get_uint64(kernel, kRPVControllerKernelId, 0,
                                                     &kernel_id) ==
                    kRocProfVisResultSuccess);
            REQUIRE(rocprofvis_controller_get_object(
                        kernel, kRPVControllerKernelPcSampling, 0, &pc_sampling) ==
                    kRocProfVisResultSuccess);
        }
    }
    ...
    if(!pc_sampling)
    {
        SKIP("The input database has no kernel with ISA data");
    }

    const auto fetch_stalls = [&](bool     include_argument,
                                  uint64_t include_instruction_samples) {
        ArgumentsPtr args(rocprofvis_controller_arguments_alloc(),
                          &rocprofvis_controller_arguments_free);
        FuturePtr    future(rocprofvis_controller_future_alloc(),
                            &rocprofvis_controller_future_free);
        REQUIRE(args != nullptr);
        REQUIRE(future != nullptr);
        REQUIRE(rocprofvis_controller_set_uint64(args.get(),
                                                 kRPVControllerPcSamplingArgsKernelId, 0,
                                                 kernel_id) == kRocProfVisResultSuccess);
        if(include_argument)
        {
            REQUIRE(rocprofvis_controller_set_uint64(
                        args.get(), kRPVControllerPcSamplingArgsIncludeInstructionSamples,
                        0, include_instruction_samples) == kRocProfVisResultSuccess);
        }
        REQUIRE(rocprofvis_controller_pc_sampling_fetch_stalls_async(
                    controller.get(), args.get(), future.get(), pc_sampling) ==
                kRocProfVisResultSuccess);
        REQUIRE(rocprofvis_controller_future_wait(future.get(), FLT_MAX) ==
                kRocProfVisResultSuccess);
        uint64_t fetch_result = std::numeric_limits<uint64_t>::max();
        REQUIRE(rocprofvis_controller_get_uint64(future.get(), kRPVControllerFutureResult,
                                                 0, &fetch_result) ==
                kRocProfVisResultSuccess);
    ...
    uint64_t sample_state_count       = 0;
    uint64_t stall_reason_count       = 0;
    uint64_t instruction_sample_count = 0;
    REQUIRE(rocprofvis_controller_get_uint64(
                pc_sampling, kRPVControllerPCSamplingNumPcSampleStates, 0,
                &sample_state_count) == kRocProfVisResultSuccess);
    REQUIRE(rocprofvis_controller_get_uint64(
                pc_sampling, kRPVControllerPCSamplingNumPcSampleStallReasons, 0,
                &stall_reason_count) == kRocProfVisResultSuccess);
    REQUIRE(rocprofvis_controller_get_uint64(
                pc_sampling, kRPVControllerPCSamplingNumInstructionSamples, 0,
                &instruction_sample_count) == kRocProfVisResultSuccess);
    REQUIRE(sample_state_count > 0);
    REQUIRE(stall_reason_count > 0);
    REQUIRE(instruction_sample_count == 0);

    fetch_stalls(false, 0);
    REQUIRE(rocprofvis_controller_get_uint64(
                pc_sampling, kRPVControllerPCSamplingNumInstructionSamples, 0,
                &instruction_sample_count) == kRocProfVisResultSuccess);
    REQUIRE(instruction_sample_count > 0);
